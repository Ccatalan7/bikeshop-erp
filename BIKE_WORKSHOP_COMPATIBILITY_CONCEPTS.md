...
- (2026-09-16) the product side of these seams lives in the successor keys of the 2026-09 templates and the engine reads them first, with the retired key as fallback: `sprocket_count` and the `cog_sequence` rows (were `drivetrain_speeds`), `cassette_spline_standard` and the `freehub_bodies_accepted` rows (`freehub_type`), `freewheel_thread_standard` / `cog_thread_standard` (a thread is the receiver), `shift_technology` (`drivetrain_platform` on cogs; bare `HYPERGLIDE` is HG/SIS, `HYPERGLIDE+` is HG+), `rear_derailleur_application_configurations` rows (`rear_derailleur_max_teeth`, `total_capacity`, speeds), `rear_derailleur_actuation_ratio_declaration` and the `*_compatibility_claims` rows (`shift_actuation_family`), `front_derailleur_application_configurations` and `front_derailleur_clamp_options` rows (`front_chainring_count`, `front_derailleur_clamp_mm`), `shifter_indexed_positions` and the `shifter_units` rows (`drivetrain_speeds` / `front_chainring_count` by side), `crank_axle_interface_declarations` rows (`spindle_interface` on cranks), `bottom_bracket_required` rows (`bottom_bracket_family` on cranks), `chain_application_declarations` and `connector_target_declarations` rows (`drivetrain_platform`, `chain_connector_target`).
- row-shaped successors arrive from the reader as the stored JSON text; a consumer decodes them and reads each row's verdict (`status`, `declaration_result` or `verdict`). A manufacturer negation («Incompatible declarado», «No compatible declarado», «Excluido por la fuente») refutes even when the nominal seams line up; a conditional row counts as affirmative with conditions.
- a rear derailleur with documented configurations and none that fits the bicycle (chainrings, sprockets, largest cog within min/max) is an enumerated caution, not an approval; a shifter's «Velocidades» belongs to its side (2–3 are chainrings, 5–13 are sprockets) and two unit rows of different sides make a pair; a friction or switchable shifter accepts any actuation family.
...
- (2026-09-16) the product side reads the successors of the brake pieces first: `rotor_size_recipe` rows (diameter, frame mount and adapter per wheel, compared with the bicycle's rotor on that wheel), `brake_model_fluid_approvals` rows and `brake_circuits` rows (`fluid_type`), `brake_actuation`, `braking_surface`, `caliper_mount_interface`, `piston_count_value`, `cable_pull_required`, `compound_type`, `pad_retention`, `compatible_caliper_models`, `lever_*`, `brake_presentation`, `rotor_mount_type`. The aggregate bicycle brake type still refutes nothing; the caution describes the piece in its own fields.
...
- (2026-09-16) the product side reads the successors first: `bb_installation_claims` rows (shell designation plus exact width or interval, with a status) and `bb_shell_ports` rows (a measured thread or press seat: 1.37" × 24 tpi is BSA, 36 × 24 Italian, 47 T47; 41 mm press is BB86/92, 42 BB30, 46 PF30/BB386, 51.5 American) replace `bb_shell_standard` / `bottom_bracket_family` / `bb_shell_width_mm`; `bb_accepted_spindles` rows and the piece's own `spindle_interface` replace `spindle_interface_accepted`. A shell conflict needs every documented configuration to conflict; a width matches when any configuration covers it; a declared incompatibility with the bicycle's shell or spindle refutes.
