...
**Production progress:** The shared operation/checkpoint kernel covers invoices, payments, POS/Quick Sale, structured adjustments, bulk parent/child rows, imports, and conversions. Workshop ownership is certified in shadow mode. Online checkout returns the original order on safe replay, MercadoPago uses server values, and provider events validate exact CLP/currency append-only. Existing anomalies were not repaired. Preserved online cancellation/refund workflow, optional future transfers/reservations, enforcement activation, and observation remain open.
