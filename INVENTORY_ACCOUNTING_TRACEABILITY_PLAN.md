...
**Production progress:** The shared operation/checkpoint kernel covers invoices, payments, POS/Quick Sale, structured adjustments, bulk parent/child rows, imports, conversions, and online-order cancellation. Workshop ownership is certified in shadow mode. Online checkout returns the original order on safe replay, MercadoPago validates exact CLP/currency append-only, and unpaid cancellation preserves the invoice plus linked reversal evidence. Paid cancellation cannot pretend to refund money and is routed to invoice Correcciones. Existing anomalies were not repaired. Manual-payment concurrency certification, optional future transfers/reservations, receipt enforcement, and observation remain open.
