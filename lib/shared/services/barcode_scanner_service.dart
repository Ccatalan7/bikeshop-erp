...
class BarcodeScannerService extends ChangeNotifier {
  ...
  void processKeyEvent(RawKeyEvent event) {
    ...
    // Escape and other control/navigation keys are never valid barcode input.
    // Clear any partial buffer so normal keyboard use cannot leak into scans.
  ...
  bool _isNonPrintableKey(RawKeyEvent event) {
    final key = event.logicalKey;
    ...
    return event.character == null || event.character!.trim().isEmpty;
  }
