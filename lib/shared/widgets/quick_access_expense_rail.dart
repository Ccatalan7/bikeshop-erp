...
class _QuickAccessExpenseRailState extends State<QuickAccessExpenseRail> {
  ...
  Future<void> _openTemplateEditor({ExpenseTemplate? template}) async {
    ...
    try {
      ...
      final keywords = <String>{
        ...
        if (_selectedSupplier != null) _selectedSupplier!.name,
  ...
  _QuickExpenseOcrResult _buildQuickExpenseOcrResult(
    ...
    return _QuickExpenseOcrResult(
      ...
      utilityKind:
          _inferUtilityKind('${parsedInvoice.supplierName ?? ''}\n$rawText'),
    );
  }
  ...
  void _applyQuickExpenseOcrResult(_QuickExpenseOcrResult ocrResult) {
    ...
    final utilityAccount = _resolveUtilityAccount(ocrResult);
    final utilityCategory = _resolveCategoryForAccount(utilityAccount);
    ...
    setState(() {
      ...
      if (utilityAccount != null) {
        _selectedAccount = utilityAccount;
      }
      if (utilityCategory != null) {
        _selectedCategory = utilityCategory;
      }
      ...
      if (canReplaceDescription) {
        _setDescriptionText(
          ocrResult.description ?? _defaultOcrDescription(parsedInvoice),
          autoGenerated: true,
        );
      }
    });
  ...
  shared_supplier.Supplier? _matchSupplierFromOcr(ParsedInvoice parsedInvoice) {
    ...
    if (parsedName.isEmpty) return null;

    for (final supplier in _suppliers) {
      final supplierName = _normalizeSearchText(supplier.name);
      if (supplierName == parsedName ||
          supplierName.contains(parsedName) ||
          parsedName.contains(supplierName)) {
        return supplier;
      }
    }
    return null;
        ...
        children: [
          ...
          if (_linkToPurchaseInvoice) ...[
            ...
            DropdownButtonFormField<PurchaseInvoice>(
              key: ValueKey(
                'purchase-link-${_selectedPurchaseInvoice?.id ?? 'none'}',
              ),
              initialValue: _selectedPurchaseInvoice,
              isExpanded: true,
              ...
              items: _purchaseInvoices
                  .map(
                    (invoice) => DropdownMenuItem<PurchaseInvoice>(
                      value: invoice,
                      child: Text(
