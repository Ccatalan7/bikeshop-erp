...
class _PaymentsPageState extends State<PaymentsPage> {
  ...
  static const String _unknownSourceValue = '__unknown__';

  final TextEditingController _searchController = TextEditingController();
  final FocusNode _keyboardFocusNode =
      FocusNode(debugLabel: 'Payments master list');
  final ScrollController _headerHorizontalController = ScrollController();
  final ScrollController _bodyHorizontalController = ScrollController();
  final ScrollController _verticalScrollController = ScrollController();
  final ScrollController _cardScrollController = ScrollController();

  List<_PaymentColumnConfig> _columns = _defaultColumns();
  List<_PaymentRow> _lastVisibleRows = const [];
  String _searchTerm = '';
  String _sortColumn = 'date';
  bool _sortAscending = false;
  _PaymentDatePreset _datePreset = _PaymentDatePreset.all;
  String _selectedMethodId = _allFilterValue;
  String _selectedSource = _allFilterValue;
  ...
  Future<void> _loadData({bool forceRefresh = true}) async {
    ...
    try {
      await Future.wait([
        salesService.loadPayments(forceRefresh: forceRefresh),
        salesService.loadInvoices(forceRefresh: forceRefresh),
        paymentMethodService.loadPaymentMethods(forceRefresh: forceRefresh),
      ]);
      await paymentMethodService.loadReferencedPaymentMethods(
        salesService.payments.map((payment) => payment.paymentMethodId),
      );

      final errors = <String>{
        if (salesService.paymentError != null) salesService.paymentError!,
        if (salesService.invoiceError != null) salesService.invoiceError!,
        if (paymentMethodService.error != null) paymentMethodService.error!,
      };
      ...
      if (!mounted) return;
      setState(() {
        _loadError = 'No se pudo cargar el registro de pagos: $error';
        _isLoading = false;
        _isRefreshing = false;
      });
    }
  ...
  Future<void> _loadPreferences() async {
    ...
    final ordered = <_PaymentColumnConfig>[];
    if (savedOrder != null) {
    ...
    ordered
        .addAll(columns.where((column) => columnsById.containsKey(column.id)));
  ...
  Future<void> _saveColumnPreferences() async {
    ...
    await Future.wait([
      ...
      preferences.setStringList(
        ...
        _columns
            .map((column) => '${column.id}:${column.width.toStringAsFixed(1)}')
            .toList(growable: false),
      ),
    ]);
  }
  ...
  Future<void> _saveListPaneWidth() async {
    final preferences = await SharedPreferences.getInstance();
    await preferences.setDouble(_listPaneWidthPreferenceKey, _listPaneWidth);
  }
  ...
  List<_PaymentRow> _buildJoinedRows(
    ...
    return salesService.payments.map((payment) {
      ...
      return _PaymentRow(
        ...
        sourceKey: sourceKey,
        sourceLabel: _sourceLabel(sourceKey),
      );
    }).toList(growable: false);
  }

  List<_PaymentRow> _filteredAndSortedRows(List<_PaymentRow> rows) {
    ...
    final filtered = rows.where((row) {
      ...
      if (_selectedSource != _allFilterValue &&
          row.sourceKey != _selectedSource) {
        return false;
      }
    ...
    filtered.sort((left, right) {
      final comparison = switch (_sortColumn) {
        ...
        'customer' => _compareText(left.customerName, right.customerName),
        'invoice' => _compareText(left.invoiceNumber, right.invoiceNumber),
        'method' =>
          _compareText(left.paymentMethodName, right.paymentMethodName),
        'reference' => _compareText(
            left.payment.reference ?? '', right.payment.reference ?? ''),
        'amount' => left.payment.amount.compareTo(right.payment.amount),
        'net' => left.payment.netAmount.compareTo(right.payment.netAmount),
        'iva' => left.payment.ivaAmount.compareTo(right.payment.ivaAmount),
        'notes' =>
          _compareText(left.payment.notes ?? '', right.payment.notes ?? ''),
  ...
  bool _matchesSmartSearch(_PaymentRow row, String normalizedQuery) {
    ...
    final values = <Object?>[
      ...
      row.payment.netAmount.toStringAsFixed(0),
      row.payment.ivaAmount.toStringAsFixed(0),
    ...
    final compactDigits = values
        .where((value) => value != null)
        .map((value) => value.toString().replaceAll(RegExp(r'[^0-9]'), ''))
        .where((value) => value.isNotEmpty)
        .toList(growable: false);
    ...
    return normalizedQuery
        ...
        .every((token) {
      if (normalizedValues.any((value) => value.contains(token))) return true;
  ...
  bool _matchesDatePreset(DateTime value, _PaymentDatePreset preset) {
    if (preset == _PaymentDatePreset.all) return true;
    final local = value.toLocal();
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    final date = DateTime(local.year, local.month, local.day);
    return switch (preset) {
      _PaymentDatePreset.all => true,
      _PaymentDatePreset.today => date == today,
      _PaymentDatePreset.last7Days =>
        !date.isBefore(today.subtract(const Duration(days: 6))) &&
            !date.isAfter(today),
      _PaymentDatePreset.thisMonth =>
        date.year == today.year && date.month == today.month,
      _PaymentDatePreset.previousMonth => () {
  ...
  int get _activeFilterCount =>
      (_datePreset == _PaymentDatePreset.all ? 0 : 1) +
      (_selectedMethodId == _allFilterValue ? 0 : 1) +
      (_selectedSource == _allFilterValue ? 0 : 1);

  void _clearFilters() {
    setState(() {
      _datePreset = _PaymentDatePreset.all;
      _selectedMethodId = _allFilterValue;
      _selectedSource = _allFilterValue;
    });
  ...
  void _reconcileSelectionAfterFiltering() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (_lastVisibleRows.any((row) => row.id == _selectedPaymentId)) return;
  ...
  void _selectRow(_PaymentRow row) {
    _keyboardFocusNode.requestFocus();
    if (!_isDesktopLayout) {
      context.push('/sales/payments/${row.id}');
      return;
    }
  ...
  void _openInvoice(_PaymentRow row) {
    final invoiceId = row.invoice?.id ?? row.payment.invoiceId;
    if (invoiceId.isEmpty) return;
    context.push('/sales/invoices/$invoiceId');
  }
  ...
  void _moveSelection(int delta) {
    ...
    final currentIndex =
        _lastVisibleRows.indexWhere((row) => row.id == _selectedPaymentId);
    final nextIndex = currentIndex < 0
        ? (delta > 0 ? 0 : _lastVisibleRows.length - 1)
        : (currentIndex + delta).clamp(0, _lastVisibleRows.length - 1);
  ...
  void _activateSelection() {
    if (_isEditableTextFocused || _lastVisibleRows.isEmpty) return;
    final selected = _lastVisibleRows
        .where((row) => row.id == _selectedPaymentId)
        .firstOrNull;
    if (selected == null) {
    ...
    context.push('/sales/payments/${selected.id}');
  }
  ...
  void _resizeColumn(_PaymentColumnConfig column, double delta) {
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final allRows = _buildJoinedRows(salesService, paymentMethodService);
    ...
    return MainLayout(
      child: Shortcuts(
        shortcuts: const {
          SingleActivator(LogicalKeyboardKey.arrowDown): _PaymentMoveIntent(1),
          SingleActivator(LogicalKeyboardKey.arrowUp): _PaymentMoveIntent(-1),
          SingleActivator(LogicalKeyboardKey.enter): _PaymentActivateIntent(),
        },
        child: Actions(
          actions: {
            _PaymentMoveIntent: CallbackAction<_PaymentMoveIntent>(
              onInvoke: (intent) {
                _moveSelection(intent.delta);
                return null;
              },
            ),
            _PaymentActivateIntent: CallbackAction<_PaymentActivateIntent>(
              onInvoke: (_) {
                _activateSelection();
                return null;
              },
  ...
  Widget _buildDesktopLayout(
    List<_PaymentRow> allRows,
    List<_PaymentRow> visibleRows,
    double availableWidth,
  ) {
    final selectedRow = _selectedPaymentId == null
        ? null
        : allRows.where((row) => row.id == _selectedPaymentId).firstOrNull;
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: canSplit
              ? _buildSplitView(
                  allRows, visibleRows, selectedRow, availableWidth)
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '$totalCount registros vinculados a facturas de venta',
  ...
  Widget _buildSplitView(
    ...
    return Row(
      children: [
        ...
        Semantics(
          label: 'Ajustar ancho de la lista de pagos',
          child: MouseRegion(
        ...
        Expanded(
          child: selectedRow.invoice == null
              ...
              : PaymentDetailView(
                  key: ValueKey('payment-detail-${selectedRow.id}'),
  ...
  Widget _buildPaymentCardsList(List<_PaymentRow> rows) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;

    return ListView.builder(
      key: const ValueKey('payments-split-card-list'),
      ...
      itemBuilder: (context, index) {
        ...
        return Material(
          key: ValueKey('payment-card-${row.id}'),
  ...
  Widget _buildPaymentMethodChip(_PaymentRow row) {
    final theme = Theme.of(context);
  ...
  Widget _buildToolbar(List<_PaymentRow> allRows, {required bool compact}) {
    final theme = Theme.of(context);
    ...
    return SizedBox(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: compact
              ? 'Buscar pagos…'
              : 'Buscar COB, factura, cliente, RUT, referencia o monto…',
  ...
  Widget _buildDateFilter(List<_PaymentRow> allRows) {
    return _buildFilterMenu<_PaymentDatePreset>(
      tooltip: 'Filtrar por fecha',
      icon: Icons.calendar_today_outlined,
      label: _datePreset.label,
      value: _datePreset,
      items: _PaymentDatePreset.values
          .map(
  ...
  Widget _buildMethodFilter(List<_PaymentRow> allRows) {
  ...
  Widget _buildSourceFilter(List<_PaymentRow> allRows) {
    final counts = <String, int>{};
    for (final row in allRows) {
      counts.update(
        row.sourceKey,
    ...
    final sources = counts.keys.toList()
      ..sort((left, right) =>
          _compareText(_sourceLabel(left), _sourceLabel(right)));
    return _buildFilterMenu<String>(
      tooltip: 'Filtrar por origen de la venta',
      icon: Icons.route_outlined,
      label: _selectedSource == _allFilterValue
          ? 'Todos los orígenes'
      ...
      items: [
        DropdownMenuItem(
          value: _allFilterValue,
          child: Text('Todos los orígenes (${allRows.length})'),
        ),
        ...sources.map(
          (source) => DropdownMenuItem(
            value: source,
            child: Text('${_sourceLabel(source)} (${counts[source]})'),
          ),
        ),
      ],
      onChanged: (value) {
  ...
  Widget _buildSummaryCards(List<_PaymentRow> rows) {
          ...
          child: compact
              ? Column(
                  children: [
                    Row(
                      children: [
                        Expanded(child: _buildSummaryMetric(metrics[0])),
                        const SizedBox(width: 16),
                        Expanded(child: _buildSummaryMetric(metrics[1])),
                      ],
                    ),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        Expanded(child: _buildSummaryMetric(metrics[2])),
                        const SizedBox(width: 16),
                        Expanded(child: _buildSummaryMetric(metrics[3])),
                      ],
                    ),
                  ],
                )
              : Row(
                  children: [
                    for (var index = 0; index < metrics.length; index++) ...[
                      ...
                      Expanded(child: _buildSummaryMetric(metrics[index])),
                    ],
                  ],
                ),
        );
  ...
  Widget _buildSummaryMetric(_PaymentSummaryMetric metric) {
    final theme = Theme.of(context);
    return Row(
      children: [
        Container(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (!compact && latest != null) ...[
            const Spacer(),
            Text(
              'Último: ${ChileanUtils.formatDateTime(latest)}',
              style: theme.textTheme.bodySmall?.copyWith(
                color: theme.colorScheme.onSurfaceVariant,
              ),
            ),
  ...
  Widget _buildPaymentsTable(List<_PaymentRow> rows) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final tableWidth =
            math.max(columnsWidth + fixedColumnsWidth, constraints.maxWidth);
        ...
        return Container(
          key: const ValueKey('payments-full-table'),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              SingleChildScrollView(
                ...
                child: SizedBox(
                  width: tableWidth,
                  child: _buildTableHeader(visibleColumns, effectiveWidths),
                ),
              ),
              Expanded(
                child: Scrollbar(
  ...
  Widget _buildTableHeader(
    List<_PaymentColumnConfig> columns,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          ...columns.map(
            (column) => _buildHeaderCell(column, widths[column.id]!),
          ),
  ...
  Widget _buildHeaderCell(_PaymentColumnConfig column, double width) {
    final theme = Theme.of(context);
          ...
          Positioned(
            ...
            child: MouseRegion(
              ...
              child: GestureDetector(
                ...
                onDoubleTap: () {
                  final defaultColumn = _defaultColumns()
                ...
                child: PopupMenuButton<String>(
                  ...
                  onSelected: (action) {
                    switch (action) {
                      case 'open':
                        ...
                        break;
                      case 'edit':
                        context.push('/sales/payments/${row.id}/edit');
                        break;
                      case 'invoice':
                        _openInvoice(row);
                        break;
                  ...
                  itemBuilder: (context) => const [
                    ...
                    PopupMenuItem(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.receipt_long_outlined, size: 17),
                          SizedBox(width: 9),
                          Text('Abrir factura'),
                        ],
                      ),
  ...
  Widget _buildTableCell(
    ...
    switch (column.id) {
      ...
      case 'net':
        child = Text(
          ChileanUtils.formatCurrency(row.payment.netAmount),
          overflow: TextOverflow.ellipsis,
          textAlign: TextAlign.right,
          style: textStyle,
        );
      ...
      case 'iva':
        child = Text(
          ChileanUtils.formatCurrency(row.payment.ivaAmount),
          overflow: TextOverflow.ellipsis,
          textAlign: TextAlign.right,
          style: textStyle,
        );
  ...
  String? _cellTooltip(_PaymentRow row, String columnId) {
    return switch (columnId) {
      'code' => row.code,
      'date' => ChileanUtils.formatDateTime(row.payment.date.toLocal()),
      'customer' => [
          row.customerName,
          if (row.invoice?.customerRut?.trim().isNotEmpty == true)
            ChileanUtils.formatRut(row.invoice!.customerRut),
        ].join(' · '),
      'method' => row.paymentMethodName,
      'reference' => row.payment.reference,
      'amount' => ChileanUtils.formatCurrency(row.payment.amount),
      'net' => ChileanUtils.formatCurrency(row.payment.netAmount),
      'iva' => ChileanUtils.formatCurrency(row.payment.ivaAmount),
  ...
  Widget _buildMobileHeader(List<_PaymentRow> allRows) {
    final theme = Theme.of(context);
    return Container(
  ...
  Widget _buildMobilePaymentCard(_PaymentRow row) {
    final theme = Theme.of(context);
    return Card(
      ...
      child: InkWell(
        onTap: () => context.push('/sales/payments/${row.id}'),
        borderRadius: BorderRadius.circular(8),
        child: Padding(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: Text(
                      '${row.paymentMethodName} · ${ChileanUtils.formatDateTime(row.payment.date.toLocal())}',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: theme.colorScheme.onSurfaceVariant,
          ...
          children: [
            ...
            Text(
              filtered
                  ...
                  : 'Los pagos registrados desde las facturas aparecerán aquí.',
              textAlign: TextAlign.center,
              style: theme.textTheme.bodyMedium?.copyWith(
                color: theme.colorScheme.onSurfaceVariant,
              ),
  ...
  Widget _buildMissingInvoiceDetail(_PaymentRow row) {
    final theme = Theme.of(context);
    return ColoredBox(
      color: theme.colorScheme.surfaceContainerLowest,
      child: Center(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 7),
                Text(
                  'El pago ${row.code} conserva la referencia ${row.invoiceNumber}, '
                  'pero no fue posible cargar su factura. Actualiza antes de operar.',
                  textAlign: TextAlign.center,
                ),
                const SizedBox(height: 16),
                Wrap(
  ...
  static List<_PaymentColumnConfig> _defaultColumns() => [
        _PaymentColumnConfig(
          id: 'code',
          label: 'Cobro',
          width: 125,
          minWidth: 105,
  ...
  static String _paymentNumber(Payment payment) {
    final id = payment.id?.trim();
    if (id == null || id.isEmpty) return 'COB-SIN-ID';
  ...
  static String _sourceKey(String? source) {
    ...
    return value == null || value.isEmpty ? _unknownSourceValue : value;
  }
  ...
  static String _sourceLabel(String source) {
    return switch (source) {
      ...
      'mechanic_job' => 'Taller',
      _unknownSourceValue => 'Sin origen informado',
      _ => source
          .split('_')
...
class _PaymentRow {
  ...
  String get invoiceNumber {
    final invoiceValue = invoice?.invoiceNumber.trim();
    if (invoiceValue != null && invoiceValue.isNotEmpty) return invoiceValue;
    final reference = payment.invoiceReference?.trim();
    return reference == null || reference.isEmpty ? 'Sin factura' : reference;
  }

  String get customerName {
    final value = invoice?.customerName?.trim();
    return value == null || value.isEmpty ? 'Cliente no disponible' : value;
  }
}

class _PaymentColumnConfig {
  _PaymentColumnConfig({
    required this.id,
    required this.label,
    required this.width,
...
enum _PaymentDatePreset {
  ...
  const _PaymentDatePreset(this.label);
  final String label;
}

class _PaymentMoveIntent extends Intent {
  const _PaymentMoveIntent(this.delta);
  final int delta;
}

class _PaymentActivateIntent extends Intent {
  const _PaymentActivateIntent();
}
