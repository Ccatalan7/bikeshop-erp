...
class StockMovementsService extends ChangeNotifier {
  ...
  Future<void> _setupRealtime() async {
    try {
      ...
      if (tenantId == null) {
        debugPrint('⚠️ [StockMovementsService] Cannot setup realtime: no tenant_id');
        return;
      }
      
      debugPrint('🔔 [StockMovementsService] Setting up realtime for tenant: $tenantId');
      ...
      _movementsChannel = _supabase
          ...
          .onPostgresChanges(
            ...
            callback: (payload) {
              debugPrint('🔔 [StockMovementsService] Stock movement changed: ${payload.eventType}');
      ...
      _adjustmentsChannel = _supabase
          ...
          .onPostgresChanges(
            ...
            callback: (payload) {
              debugPrint('🔔 [StockMovementsService] Stock adjustment changed: ${payload.eventType}');
              // Reload if we have a product selected
              if (_selectedProductId != null) {
                loadMovementsForProduct(_selectedProductId!);
              }
  ...
  Future<void> loadMovementsForProduct(String productId) async {
    ...
    try {
      ...
      _error = null;
      debugPrint('✅ [StockMovementsService] Loaded ${_movements.length} movements for product $productId');
    } catch (e) {
      _error = e.toString();
