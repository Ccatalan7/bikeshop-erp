  ...
  Future<List<models.Category>> getCategories({
    ...
    try {
      ...
      if (searchTerm != null && searchTerm.isNotEmpty) {
        ...
        final nameResults = await _db.searchRecords('categories', 'name', searchTerm);
        final descResults = await _db.searchRecords('categories', 'description', searchTerm);
      ...
      List<models.Category> categories = data.map((json) => models.Category.fromJson(json)).toList();
  ...
  Future<Map<String, dynamic>> getCategoryAnalytics() async {
    try {
      ...
      for (final category in categories) {
        final products = await _db.select('products', where: 'category_id=${category.id}');
  ...
  Future<void> initializeDefaultCategories() async {
    try {
      ...
      if (existingCategories.isNotEmpty) {
        if (kDebugMode) print('Categories already exist, skipping initialization');
        return;
      }
      
      final defaultCategories = [
        models.Category(name: 'Bicicletas', description: 'Bicicletas completas de todos los tipos'),
        models.Category(name: 'Repuestos', description: 'Piezas y componentes para bicicletas'),
        models.Category(name: 'Accesorios', description: 'Accesorios y complementos para ciclistas'),
        models.Category(name: 'Ropa', description: 'Vestimenta y equipamiento para ciclistas'),
        models.Category(name: 'Herramientas', description: 'Herramientas para mantenimiento y reparación'),
        models.Category(name: 'Mantenimiento', description: 'Productos para mantenimiento y limpieza'),
        models.Category(name: 'Otros', description: 'Productos diversos no clasificados'),
      ];
