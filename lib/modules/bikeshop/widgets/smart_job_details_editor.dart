...
class SmartJobDetailsEditor extends StatefulWidget {
  ...
  @override
  State<SmartJobDetailsEditor> createState() => _SmartJobDetailsEditorState();
}

enum _JobDetailTab { solicitud, diagnostico, trabajos, notas }

class _SmartJobDetailsEditorState extends State<SmartJobDetailsEditor> {
  ...
  @override
  void didUpdateWidget(SmartJobDetailsEditor oldWidget) {
    super.didUpdateWidget(oldWidget);
    if ((!_isOpen && !widget.isInline) ||
        (widget.isInline &&
            (oldWidget.job?.id != widget.job?.id ||
                oldWidget.jobBike?.id != widget.jobBike?.id))) {
      _initValues();
      if (widget.isInline) {
        _textController.text = _getCurrentFieldValue();
      }
  ...
  void _showOverlay() {
    if (_isOpen || widget.isInline) return;
  ...
  Widget build(BuildContext context) {
    if (widget.isInline) {
      final theme = Theme.of(context);
      final isDark = theme.brightness == Brightness.dark;
      return _buildEditorContent(isDark, isPopup: false);
    }

    // Original trigger widget behavior
    return _buildTriggerWidget();
  }

  Widget _buildEditorContent(bool isDark, {required bool isPopup}) {
    final theme = Theme.of(context);
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        // Header (Customer/Bike) - Only for Popup
        if (isPopup && (widget.customerName != null || widget.bikeName != null))
          Container(
  ...
  void _saveChanges() {
    ...
    if (clientRequestChanged ||
      ...
      // Update originals to prevent repeated saves
      _originalClientRequest = _clientRequest;
      _originalDiagnosis = _diagnosis;
      _originalWorkPerformed = _workPerformed;
...
class _SmartDetailsExportButton extends StatelessWidget {
  final IconData icon;
