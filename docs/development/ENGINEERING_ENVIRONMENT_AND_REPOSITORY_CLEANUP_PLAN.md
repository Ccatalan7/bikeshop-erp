...
| 6 — Repository cleanup | In progress | Confirmed caches, virtual environments, logs, and screenshot archives were removed from Git while preserved locally. Root SQL is now fully classified: invalid/tenant-specific probes were removed, read-only diagnostics were retained, and every mutating/historical script was losslessly quarantined as recovery or archive evidence. Script/doc/Dart provenance batches remain. |
