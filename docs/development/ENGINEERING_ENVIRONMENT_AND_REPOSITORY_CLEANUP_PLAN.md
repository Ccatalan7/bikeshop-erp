...
| 3 — Staging/database | In progress | Lean staging contract is mandatory; canonical hosted apply passed with unchanged business counts; local/staging application drift is zero; first-pass view idempotency is regression-tested. Production/canonical historical drift remains classified but unresolved. |
