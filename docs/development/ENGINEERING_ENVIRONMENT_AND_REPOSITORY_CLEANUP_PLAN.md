...
| 4 — Test ladder | In progress | Secret and ERP integrity workflows exist; the complete 724-assertion pgTAP suite passes; Playwright verifies routed inventory/sales/purchases and now performs a deterministic manual stock adjustment plus exact reversal on the isolated staging tenant, followed by database proof of continuous stock, balanced journals, completed traces, and zero linked inconsistencies. Remaining mutating document/payment journeys and native coverage remain. |
...
| 6 — Repository cleanup | In progress | Generated waste and screenshots were removed; root SQL is fully classified; dated handoffs plus completed fix/migration summaries are archived; the final unreferenced Python source-rewrite helpers were removed. Living-doc authority and Dart provenance batches remain. |
