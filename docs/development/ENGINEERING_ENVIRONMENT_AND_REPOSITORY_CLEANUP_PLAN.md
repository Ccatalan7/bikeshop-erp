...
| 6 — Repository cleanup | In progress | Confirmed caches, virtual environments, logs, and screenshot archives were removed from Git while preserved locally; the first root-SQL batch removed invalid/tenant-specific probes and relocated reusable read-only diagnostics. Mutating SQL plus script/doc provenance batches remain. |
