...
**Actualizado 2026-08-02. Este bloque es la autoridad vigente y reemplaza
cualquier estado, PID, conteo o pendiente contradictorio del cuerpo histórico.**

> **Sesión nativa canónica:** `screen payroll`, app **`89180`**, VM
> **`:56556`**, brillo **Oscuro**, ventana **1360×800**. Está viva y no se
> inicia una segunda. Convive con la copia instalada `81847`; las acciones se
> envían siempre al proceso debug por identidad.
>
> **Criterio:** Design manda el aspecto. La app, el backend y el dominio mandan
> función, palabras, navegación, UX y reglas. Cada propuesta pasa la compuerta
> de seis dimensiones; un frame no se copia a ciegas.
>
> **Límites:** sin writes de producción, sin commit, push, deploy ni publicación.
> Las tres migraciones nuevas siguen preparadas pero NO desplegadas:
> `20260801173000_report_payroll_reconciliation_replay.sql`,
> `20260801180000_set_employee_payroll_payment_method.sql` y
> `20260801183000_add_structured_employee_advance_audit.sql`.

### VIGENTE — reconsultado 2026-08-02

| Qué | Estado comprobado |
|---|---|
| Rama · HEAD | `smartpegas1.0` · `e787754bc4bd`; `origin..HEAD = 0` |
| Árbol | **11 rutas modificadas + 1 documento untracked.** `payroll_redesign_page.dart` se sumó a las 10 preexistentes al pasar el tier a un solo owner. El untracked es `docs/development/PAYROLL_5N_TRANSITION_MATRIX_2026-08-02.md`. Sin proceso local de publicación |
| Sesión | app `89180`, VM `http://127.0.0.1:56556/3GGO0eJPJyI=/`, oscuro, 1360×800, pantalla Semanas y sin borrador OCR |
| Verificación actual | **BATERÍA ANCHA VERDE — 2026-08-02, tras corregir las 3 fallas.** Las **42 suites** del comando canónico: **570 pasadas · 2 opt-in saltadas · 0 fallidas** (`All tests passed!`, leído del texto). Compuertas enfocadas (arquitectura de tema + silueta de carga + cola adaptativa + sólo lectura + superficie + conciliación): **192 · 2 · 0**. Analyzer de `lib/modules/hr/payroll/` + las tres pruebas tocadas: **0 issues**. `git diff --check`: limpio. **Historia, porque el número anterior engañaba:** esta fila decía `163 · 0 fallidas` y era de **tres** suites, no del módulo; con las 42 había **3 rojas** heredadas, ahora corregidas (ver el bloque de abajo). **También estaba obsoleto el «33 suites / 364 pruebas»** que este documento cita más adelante: son **42 / 572** con el mismo comando |
| `5m` | **Cerrado en el owner real** `_MoneyBar`: a 834 la única acción visible se apila bajo el total a `46` exactos; cubiertos los estados confirmar-only y siguiente-only, y escritorio no se apila |
| OCR pasos 2 y 4 | **Cerrados en la app real, producción, claro, 1360/834/430, sin aplicar ni escribir.** Las 10 preguntas se resolvieron; el paso Aplicar quedó bloqueado por 4 pagos en efectivo manuales. Se salió sin guardar y se restauró oscuro |
| Identidad OCR | `PayrollDecisionOptionCard` conserva el `Semantics.label` hablado y usa `Semantics.identifier` + `ValueKey` estable `payroll-ocr-decision-<movementId>-<optionName>`. La regresión usa dos preguntas reales, resuelve una key, avanza y prueba que la misma posición pertenece a otro movimiento |
| Evidencia durable | **Ruta absoluta, porque hay DOS carpetas con el mismo nombre:** `/Users/Claudio/Dev/bikeshop-erp/Screenshots vinabikeProject/payroll-closure-2026-08-02`, dentro del repo e **ignorada** (`.gitignore:110`, confirmado con `git check-ignore -v`). Contenido verificado por SHA-256 el 2026-08-02: `01..06` OCR claro, `07-restaurado-oscuro-1360.png` (`f5f972f8…`), y **las tres de `5n` SÍ existen** — `08-5n-wide-live.png` (`69eb97e0…`), `09-5n-compact-live.png` (`e78f17b7…`), `10-5n-tablet-live.png` (`6a45a1ea…`). **Cero hashes duplicados.** Añadidas hoy, verificadas por hash y dimensión: `11-5l-oscuro-430-live.png` (`9daefb60…`, 430×896), `12-5l-claro-430-live.png` (`11a4a77b…`, 430×896), `13-5l-claro-402x827-live.png` (`041a68a0…`, 402×827), `14-restaurado-oscuro-1360-live.png` (`f5f972f8…`, 1360×768). **`14` y `07` comparten hash, y eso es la prueba, no un defecto:** son dos capturas independientes —02:52 y 04:31, sesiones distintas— del mismo estado canónico restaurado (oscuro · 1360×800 · Semanas · sin drawer), así que salir idénticas demuestra que la restauración fue exacta. ⚠️ **`~/Screenshots vinabikeProject/` es OTRA carpeta, vieja y que no cita nadie**; no se usa. Ver la corrección fechada de abajo |
| `5n` | **ADJUDICADO, NO CERRADO.** Las 26 filas están recuperadas y adjudicadas una por una en [`PAYROLL_5N_TRANSITION_MATRIX_2026-08-02.md`](PAYROLL_5N_TRANSITION_MATRIX_2026-08-02.md). Lo soportado que quedaba pendiente **está implementado y probado**: el owner del tramo subió al `LayoutBuilder` de la superficie —descontando padding lateral (32/36) **y** los 2 px del borde—, el host dejó de tener un `1240` rival, y `maxWidth 186/168/200` alcanza las cuatro formas. **Verificado vivo en la app real** (2026-08-02, producción, oscuro, sesión `payroll`/`89180`, reload 38/5159 en 2,811 s, sin abrir una segunda sesión): 1250×768 banda ancha de 8 columnas, 1200×768 tramo compacto de 6, 834×768 cuatro columnas con CTA apilado, y la fila abierta sobrevivió al redimensionar 834→1250. Capturas `08`–`10` bajo `Screenshots vinabikeProject/payroll-closure-2026-08-02/`. **No se cierra** porque quedan PARCIALES sin evidencia ejecutada (filas 9, 12, 14, 16, 18) y porque la **escritura real del paso 4 del OCR sigue no verificada** (fila 24) |
| `5l` | **CERRADO EN macOS EN LOS DOS BRILLOS (2026-08-02); la mitad iOS queda BLOQUEADA POR EL TOOLCHAIN DEL HOST, no por el código.** Claro verificado vivo frame a frame a **430** y a **402×827** (sesión `89180`, producción, sin escribir). El cromo mide **nav 56 + borde 1 + banda de alcance 48 = 105**, leído de la semántica viva, no de un píxel. **Corrección de redacción:** el `151` de `6a` **no** es «status 47 + nav 48 + título» — la anotación del propio frame dice `Chrome fijo total: 47 + 56 + 48 = 151 de 844`. Los `48` son (a) los botones de icono **dentro** de la banda de 56 y (b) la altura de la banda de pills. Con eso, lo único que falta para el 151 es el inset de **47** del sistema, que macOS no tiene. Pills: **figura 34 · objetivo 48**, confirmado en código (`SizedBox(height: 48)` + `minHeight: 34`) y vivo (`86×48`, `84×48`, `88×48`). CTA **48 exacto** en vivo (`376×48` la revelación de persona, `404×48`/`376×48` `Confirmar semana`); el **50** que dibuja `6a` sigue **RECHAZADO por `F-06`**. **Registros a 402×827: CUATRO**, y los cuatro *alcanzables* —comprobados uno a uno con `find`, que sólo devuelve lo tocable—, no heredados del 390. Cero `RenderFlex overflow` en el log de la sesión. Capturas nuevas y **distintas entre sí**: `11-5l-oscuro-430-live.png`, `12-5l-claro-430-live.png`, `13-5l-claro-402x827-live.png` |
| `5k` | **`sólo lectura` CERRADO con owner canónico y regresión propia (2026-08-02); `permiso` sigue ABIERTO por dominio; `error` sigue BLOQUEADO por ownership.** La rama `PayrollMethodAuthority.readOnly` ya estaba implementada y **no tenía una sola prueba** —el único aserto del repositorio pasaba `editable`—, así que un refactor podía volver editable la hoja de quien no puede escribir con toda la batería verde. Cubierta ahora por `test/widgets/payroll_method_read_only_authority_test.dart` (**13**), por conducta: sin capacidad no hay `Guardar`, la salida se llama `Cerrar`, el motivo lo dice **`E-04 · VbNotice`** una vez, y **tocar otra opción no mueve la selección**. Ese último aserto lleva **control positivo** —con capacidad el mismo toque SÍ abre los campos de cuenta— porque «no hay campos» se cumple solo también cuando no hay nada montado (§5.c). **Y cubre la UNIÓN, que era el hueco de verdad** (revisión de Codex, 2026-08-02): inyectar el enum prueba la hoja, **no la decisión**. La derivación salió de la página a **`payrollMethodAuthorityFor(CurrentUserProfileService?)`** —que recibe el servicio **entero**, y ésa es la parte que importa: una primera versión tomaba cuatro `bool` y entonces la página y la prueba escribían **cada una su mapeo**, así que la prueba pasaba aunque el de producción se torciera (P2 de Codex, 2026-08-02)—, y las cinco celdas se prueban llamando esa misma función con un `CurrentUserProfileService` **real** (subclaseado, con gateway falso) — admin → `editable`; **contador** → `readOnly`, con dos asertos que muestran por qué existe el caso (`canAccessAccounting` sí, `canManageUsers` no); y **servicio ausente / cargando / con problema de carga** → `readOnly`, porque un permiso que aún no se sabe no es un permiso. Con `Transferencia` preseleccionada se prueba además que **banco y número quedan deshabilitados** y que el **selector de TIPO no despliega**: se toca el control `S-05` por su key estable (`payroll-method-account-type`), no el rótulo, y se afirma que `Cuenta de Ahorro` —que sólo existe dentro de la lista abierta— no aparece; con control positivo en `editable`, donde el mismo toque sí la abre. Verificado por **mutación**: al quitar las guardas de `isLoading`/`loadIssue` la suite se pone roja en esas dos celdas. No se fabricó `PayrollAbility` ni permiso por acción: la capacidad usada es la canónica de §21bis (`can_manage_tenant_hr` para escribir vs `can_manage_tenant_payroll` para operar y leer). El **rótulo de rol** («Tu rol: Asistente») sigue sin fuente y sigue abierto (§21) |
| `5e/5f` | El composer de pago y la entrega en efectivo necesitan una semana en estado que producción no tiene sin escribir. El harness es QA suplementaria, no evidencia real |

> **Por qué el cache traía `{{ m.frame }}` — causa, no cache corrupto.** El
> campo `copy` de `spec.json` transcribe el DOM del *template*, y las 26 filas
> se generan por binding: ningún reintento sobre `spec.json` las va a producir.
> Y `Nóminas - Rediseño.dc.html` está ordenado del turno más nuevo al más viejo,
> así que los 256 KiB se agotan en `t7`+`t6` y **no llega ni un `id="5*"`**.
> La vía que sirve son los PNG por frame de `handoff-t5/frames/`. Detalle
> completo, hashes y espejo `.tmp/design-mirror/t5/` en el documento enlazado.

### Auditoría independiente que gobierna `5n`

Las seis decisiones del frame quedan, hasta revisar la fuente real:

1. Horas no cerradas fuera del total: **parcial**; total cero no prueba por sí
   solo horas sin cerrar.
2. Tolerancia fija global ±$1.000: **falla**; el servicio usa 1 %, mínimo $1,
   máximo $500, y la subpaga es asimétrica.
3. Pagar fuera de orden con la más antigua sugerida: **cumple en código**, pero
   falta una regresión enfocada.
4. `Sin método` sólo para esta semana y `Configurar` permanente: **falla**;
   hoy configurar escribe la preferencia canónica del trabajador.
5. Umbrales 90/70 sin aplicación automática: **parcial/adaptada**; sí existe
   score 0–100, pero no esa política. Toda aplicación exige confirmación humana.
6. Anticipo no automático: **cumple la seguridad**, pero no toda la propuesta
   visual de preselección reversible.

También siguen abiertos: `PaymentDraft` no persiste al desmontar; el teclado no
implementa todo el contrato propuesto; el ancho de decisión tablet llega a
`280` (no es un `maxWidth 200`); y «una superficie por archivo» es falso.

### Próximo trabajo, en orden

1. Abrir un chat Claude nuevo sólo después del preflight visible: **Code · Opus
   5 · Effort Ultracode · Remote Control ON**.
2. ~~Recuperar las 26 transiciones auténticas de `5n` desde Design~~ **hecho el
   2026-08-02** (ver la matriz enlazada). Lo que queda es **adjudicar las 26
   filas** contra negocio/backend/navegación/responsive y corregir únicamente lo
   soportado. Adjudicar es trabajo pendiente, no hecho.
3. ~~Cerrar `5l` en claro y con iOS Simulator; luego resolver lo posible de `5k`
   sin inventar autorización.~~ **Hecho hasta el límite honesto el 2026-08-02.**
   `5l` cerrado en macOS en los dos brillos; `5k · sólo lectura` cerrado con
   prueba. Lo que queda de `5l` **no es trabajo de UI**: ver el bloqueo de
   toolchain de abajo.
4. Repetir batería/analyzer y verificar la app real. **No repetir OCR 2/4** salvo
   que un cambio posterior afecte esas superficies.

### Las 3 fallas de la batería ancha: causa y corrección (2026-08-02)

**Corregidas, sin ablandar ningún contrato.** Las dos primeras eran un mismo
defecto y la tercera era la silueta, no la barra.

**(a) `5m` adjudicado contra `F-06`: el 46 de Design no puede ser el objetivo
táctil.** `PayrollAccentAction.height` dimensiona el `InkWell` entero, así que
`height: 46` es un objetivo de **46** y `F-06 · TOUCH` pide **48**. Es la misma
regla que `5l` ya aplicó al bajar su CTA de 50: **la altura dibujada no manda
sobre la regla táctil**, y acá la adaptación es *más* fiel que el número,
porque la nota 04 pide justamente que el botón no quede chico de tocar («nada
de un botón de 34 perdido en una esquina táctil»). El CTA apilado usa ahora el
owner canónico **`PayrollTokens.touchMobile`**, y el estático
**`ctaHStackedBar` se eliminó**: no era autoridad, y añadirlo al allowlist del
candado habría sido comprar comodidad con la invariante. La prueba de `5m`
conserva **igualdad exacta**, ahora contra `touchMobile`.

**(b) El salto de 1 px NO era de la barra: era de la silueta.** Medido, no
supuesto — la barra real va de 843 a 900 (**57**) y el fantasma de 844 a 900
(**56**). La causa: `moneyBarH` es el alto del **contenido** y el borde de 1 se
le suma —así lo monta la barra real (`SizedBox(height: moneyBarH)` dentro de un
`Container` con `border: top`) y así lo escribe Design, porque en `6a` las
bandas declaran `height:56px` **más** `border-bottom:1px` y la suma del cromo
(`47 + 56 + 48 = 151`) no cuenta bordes—. La silueta ponía el `moneyBarH` como
alto del `Container`, o sea **borde incluido**, y prometía 56 donde iban a caer
57. Se corrigió **la silueta**, que era la que mentía, y **no** la barra que ve
el operador: el producto no cambió un píxel. El aserto sigue siendo igualdad
exacta (`moneyBarH + 1`, con el 1 nombrado como el ancho de `BorderSide` y el
token a la vista para que un cambio siga rompiendo la prueba).

> **Lo que esto deja escrito**, y es lo que costó encontrarlo: el mensaje decía
> «la barra de dinero cambia de altura» y **el culpable era el fantasma**. Antes
> de tocar el widget que nombra el aserto, **mide los dos**. Una sonda de tres
> `print` sobre los dos rectángulos dio la causa en un intento; razonar sobre el
> texto del fallo llevaba directo a encoger la barra real 1 px.

<details>
<summary>Cómo se veían las tres antes de corregirlas (histórico)</summary>

### ⛔ La batería ANCHA está ROJA, y lo estaba antes de esta ronda (2026-08-02)

**Tres pruebas fallan, ninguna es de esta sesión, y ninguna se veía** porque las
últimas rondas sólo recorrían **tres** suites (`payroll_queue_surface_adaptive`
+ `payroll_reconciliation_responsive` + `payroll_redesign_surface`). Ése es el
`163 · 0 fallidas` de la fila «Verificación actual»: es verdad **de esas tres**,
y se leyó como si fuera del módulo. La batería completa —la que este mismo
documento manda correr— dice otra cosa.

Las tres nacen de **cambios en vuelo de otro owner**, no del código publicado:

1. **`ctaHStackedBar` rompe el candado de arquitectura de tema**, dos veces
   (`payroll_theme_architecture_test.dart`).
   `Expected: empty · Actual: Set:['ctaHStackedBar']` →
   *«New PayrollTokens statics are forbidden; add the value to the canonical
   theme pipeline instead»*, más dos usos marcados en
   `payroll_queue_surface.dart:2538` y `:2550`. El estático **no está en `HEAD`**
   (`git show HEAD:…payroll_tokens.dart` no lo tiene): entró con el trabajo de
   `5m`, que la matriz declara **«CERRADO (2026-08-01)»**. El valor `46` sí tiene
   fuente en Design; lo que el guard rechaza es **dónde vive**, no cuánto mide.
2. *(la misma suite falla dos veces: el inventario congelado y el consumo de
   roles montados; es un solo defecto con dos asertos.)*
3. **La barra de dinero salta 1 px entre carga y datos**
   (`payroll_loading_skeleton_test.dart:216`, «escritorio · el control de
   decisión NO salta al llegar los datos»): `Expected: <844.0> · Actual: <843.0>`.
   Es exactamente el defecto que esa prueba existe para atajar, y su owner es el
   mismo `_MoneyBar` que `5m` tocó.

**No se corrigieron acá, a propósito.** Son archivos en vuelo de otro owner
(`payroll_tokens.dart`, `payroll_queue_surface.dart`) y el primero es una
**invariante ancha** —quién puede crear valores visuales—, o sea material de la
compuerta de doble diagnóstico, no de una corrección mecánica del lead activo.

> **La lección, que es de proceso y no de código:** una batería recortada a las
> suites que uno tocó **no dice que el módulo esté verde**, y citarla como si lo
> dijera es la misma clase de afirmación falsa que este documento persigue desde
> `5d`. Un conteo se cita con **su fecha, su árbol y su alcance**.

</details>

### El iOS Simulator está BLOQUEADO por el toolchain del host (2026-08-02)

**No es un problema de la app ni del código: la app nunca llegó a compilar.**
El esquema `Runner` **no tiene ni un destino elegible**, comprobado con la
pregunta directa:
...
→ devuelve **sólo** el marcador de dispositivo, con su causa:
`{ platform:iOS, … error:iOS 26.5 is not installed. Please download and install
the platform from Xcode > Settings > Components. }`

Estado medido del host: **Xcode 26.6** (SDK `iphonesimulator26.5` presente en
disco) y **un único runtime instalado, iOS 26.2**. El iPhone 17
(`8568B469-B97F-476A-8288-C4F49338FD16`) **existe y está `Booted`** —el panel
adjunta bien y da su espacio de 402×874—, pero `xcodebuild` no lo enumera, así
que **fallan por igual** `flutter build ios --simulator` (destino genérico) y el
build headless con `id=` explícito. En el simulador **no hay ninguna build
previa de la app** (`simctl listapps` no trae `com.vinabike.erp`), así que
tampoco hay un atajo por una instalación vieja.

**Lo arregla el dueño, no el agente:** instalar la plataforma iOS desde
*Xcode > Settings > Components* (o `xcodebuild -downloadPlatform iOS`). Pide
contraseña y descarga varios GB — está fuera de lo que un agente hace solo.

**Qué queda esperando exactamente**, para que nadie repita el diagnóstico: sólo
el **inset de 47** de la barra de estado del sistema. Todo lo demás del `151`
—`nav 56` + `banda de alcance 48`— ya está medido en la app viva (fila `5l`).
El adjunto al panel y el espacio de puntos **no** son evidencia de la app: son
del simulador vacío.

### Entregado a Codex, sin editar (2026-08-02)

Se cumple «antes de editar, entrega evidencia + owner + solución» y **no se
tocó** el archivo:

1. **Los dos números del pill móvil son literales sin dueño.**
   `payroll_redesign_page.dart:4924` escribe `height: 48` y `:4932`
   `minHeight: 34`. **Los valores son los correctos** —el objetivo táctil y la
   figura del frame—, pero el `48` tiene owner canónico
   (`PayrollTokens.touchMobile`) y no lo cita, y el `34` no tiene ninguno.
   *Solución propuesta:* enlazar el `48` a `PayrollTokens.touchMobile` y darle
   al `34` su propio token junto a `ctaH`, con la cita del frame. Es mecánico y
   sin efecto visual; se deja para que no colisione con la ronda de Codex sobre
   ese mismo archivo.
2. **`ios/Podfile.lock` quedó modificado y NO es de nadie más.** Lo cambió el
   `pod install` de mi intento de build (21 líneas, **sólo adiciones**:
   `flutter_inappwebview_ios`, `flutter_secure_storage_darwin`, `OrderedSet`).
   O sea el lock **estaba desactualizado** respecto de `pubspec`. No se revirtió
   porque el guard del repositorio —con razón— rechaza descartar rutas en un
   árbol compartido. Se regenera solo en cualquier build de iOS: **decidir si se
   conserva o se descarta es de Codex**, no una urgencia.
...
| **5l** tarjeta móvil | sí | `5l-1-p1/p2` · `5l-2` | **n/r** | **sí · vivo** | n/a | n/a | **sí · vivo** | **sí · vivo** | sí · 2 a 390 px | **PARCIAL (2026-08-02).** Su CTA bajó de 50 a `touchMobile` (**48**) el 01/08 — el frame dibuja 50 y manda `F-06 · TOUCH`—: el **48 queda probado estructuralmente** con igualdad exacta al token, pero **la acción no fue alcanzable visualmente con datos de producción**, porque sólo se dibuja en una semana ya confirmada y llegar ahí exigiría una **escritura real**. RT queda en `no`, no en «parcial»: no hay captura de esa altura y no se inventa una **MARCO DEL TURNO 6 VERIFICADO EN VIVO, pero SÓLO EN OSCURO Y SÓLO EN macOS (2026-08-02)** a 430 contra producción (sesión `89180`), que era lo único que faltaba. Lo que el turno 6 reemplaza está: **header con título y contexto** (`Nóminas` + `S27 · 4 por resolver · $225.000`), **tres pills** `Semanas 4 · Historial • · Anticipos •` con objetivo táctil de **48** medido por semántica, y **OCR en el overflow `•••`** en vez de tres botones grandes. Lo que el turno 6 **conserva** de `5l` también: composición de la tarjeta de persona, CTA a ancho completo y **los cuatro registros completos en el primer viewport**. **Divergencia de plataforma declarada:** el `151` del frame es `barra de estado 47 + nav 48 + título`; en macOS **no hay barra de estado del sistema**, así que el header mide lo que mide y el 47 no aplica. Se verificará en el simulador iOS cuando toque. Captura: `<scratchpad>/shots-5l/movil-430.png` **Lo que NO está cerrado y por qué:** el **claro** a 430 se capturó en el mismo recorrido pero no se comparó frame a frame con el turno 6; y la **barra de estado de iOS** —los 47 px que el `151` del frame incluye— **no existe en macOS**, así que sólo el simulador iOS puede cerrarla. Evidencia durable en `~/Screenshots vinabikeProject/payroll-closure-2026-08-02/` (`08-5l-claro-430.png`, `09-5l-oscuro-430.png`) **· CORRECCIÓN FECHADA (2026-08-02).** Esa cita `~/Screenshots…` apunta a una carpeta **equivocada**: la evidencia vive en `/Users/Claudio/Dev/bikeshop-erp/Screenshots vinabikeProject/payroll-closure-2026-08-02` y **ahí no hay `08-5l-*` ni `09-5l-*`** — el `08..10` de esa carpeta son las tres de `5n`. *(Una revisión mía anterior dijo que los archivos de `5n` no existían y que había duplicados: era falso, y venía de auditar la carpeta homónima de `$HOME`. Queda retractado.)* Lo que sí faltaba —el **430 en los dos brillos, comparado frame a frame**— se hizo hoy contra producción y está verificado por hash y dimensión: `11-5l-oscuro-430-live.png` (430×896), `12-5l-claro-430-live.png` (430×896) y `13-5l-claro-402x827-live.png` (402×827) para el tamaño real del iPhone 17. **El claro sí quedó comparado frame a frame con el turno 6**, y el cromo se midió por semántica: `nav 56 + borde 1 + alcance 48`. **También se corrige la lectura del `151`:** no es «status 47 + nav 48 + título», es **`47 + 56 + 48`**, escrito por la anotación del propio `6a`. **Registros a 402×827: cuatro, los cuatro alcanzables** con `find`. Lo único que sigue abierto es el inset de **47**, y **no por trabajo de UI**: el toolchain del host no compila para iOS (ver el bloque «El iOS Simulator está BLOQUEADO» del encabezado) |
| **5m** tablet 834 | **sí** | `5m-p1/p2` | **sí** | **sí** | n/a | **sí · vivo** | n/a | **sí · vivo** | sí · **3** en `payroll_queue_surface_adaptive_test` | **CERRADO (2026-08-01).** El hueco no era visual sino de host: **a 834 la página montaba las tarjetas de teléfono**, no la tabla de 5m. El frame separa las dos preguntas y su nota 03 lo dice: bajo 900 el **chrome** es compacto —header único con drawer, «el contrato de 3c se mantiene intacto»—, pero el **contenido** sigue siendo la tabla. Ahora el host tiene banda de tablet (`_isTabletBand`, piso 720) y monta `PayrollQueueSurface`; el tier ya existía en la superficie y **nadie lo instanciaba**. Implementado además lo que la banda pedía y no estaba: **fila de 60** y el **mismo** control de decisión a **44 × 200** —nota 02 del frame, «sin cambiar de forma ni de verbo»—, y la barra monetaria de **una sola acción** se apila (nota 04). **Verificado vivo a 834 en claro y oscuro** contra los 4 borradores reales: cuatro columnas `PERSONA · TOTAL · A PAGAR · DECISIÓN`, sin `ANTICIPOS`, sin `PAGADO`, sin `MÉTODO`. **CERRADO DEL TODO el 2026-08-02, y la deuda que quedaba era FALSA en el peor sentido: la barra apilada estaba implementada en un widget que esta superficie NUNCA monta.** El handoff la daba por «hecha, sin confirmar en vivo»; al mirarla en la app seguía lado a lado. Causa: el apilado vivía en `PayrollMoneyBar`, y la cola tiene **su propia** `_MoneyBar`. Reimplementado donde se pinta, con la nota 04 literal —«cifra y razón arriba, botón de **46** abajo; nada de un botón de 34 perdido en una esquina táctil»— y sólo con **una** acción: con dos, apilar dejaría dos primarios uno sobre otro. **Verificado vivo a 834** (sesión `89180`): `FALTA $225.000` arriba, `Confirmar semana` a ancho completo abajo. Dos contratos nuevos con mutación probada. **La lección:** «implementado» no es «montado» — un contrato que no mide el widget que se pinta puede estar verde sobre código muerto |
...
| **5k** siete estados | **parcial** | `5k.png` íntegro (191.845 B) | **sí** | **sí** | **sí** | **sí** | **sí** | **sí** | sí · 9 + 9 | **`carga` CERRADO** como `X-01 · VbSurfaceState` (`lib/shared/widgets/vb_skeleton.dart`) + `_PayrollLoadingSkeleton` en el gate de `payroll_redesign_page.dart`. **Las SEIS celdas capturadas en la app viva contra producción**: claro y oscuro × 1360 / 834 / 430, cada una con el esqueleto **en pantalla** —no el estado cargado, no el drawer, no el Dashboard—. **Las cuatro compactas** (claro y oscuro × 834 y × 430) son de la sesión `69949`/`:59077` —**PID histórico**, como `1320`/`:51308`; la sesión viva se lee de la tabla VIGENTE del encabezado— y **siguen vigentes**: la silueta compacta dibuja tarjetas, no tabla, así que no depende de `ghostRowsFor`. **Las DOS de escritorio se volvieron a capturar en `83620`/`:62489`** (también histórico) —oscuro y claro, las dos— después de derogar el techo de filas, porque ese conteo es exactamente lo que muestran: sin tope la tabla llena el hueco hasta la barra de dinero (**13 filas** a 1360×789) en lugar de cortarse en 6 y dejar un vacío. Para la clara se cambió el brillo y **se devolvió a `Oscuro`**, comprobado con el check en su sitio. Ruta de las capturas vigentes: `celda-oscuro-escritorio-sintecho.png`, `celda-claro-escritorio-sintecho.png`, `celda-oscuro-compacto.png`, `celda-oscuro-movil.png`, `celda-claro-tablet.png`, `celda-claro-movil.png`, en el scratchpad de la sesión. **`vacío` también HECHO**: uno general honesto que ya no afirma la causa, más «Aún no hay nadie contratado» con salida a Trabajadores — la única causa que el esquema garantiza (§4.13). **No es alcanzable en vivo sin escribir** (§4.19), así que va por contrato. **`error` diagnosticado y bloqueado por ownership** (§4.20). **`permiso` NO está implementado y NO es derivable** (§4.21): esta superficie no tiene owner ni capacidad de rol, `blockedReason` **no es RBAC** y no cuenta como cierre de permiso. Con eso, **5k está cerrado hasta donde una migración visual puede llegar, y el estado de permiso queda ABIERTO con nombre**. Cola local y `Deshacer` **descartados** con razón (ledger 01/08) **EL ESTADO «SIN PERMISO» QUEDA EXPLÍCITAMENTE ABIERTO, y la causa es de modelo, no de UI (2026-08-02).** `5k` y `5n` piden que la celda de decisión reciba `enabled` + `disabledReason` y **se inhabilite con motivo, jamás se oculte**. La mitad visual ya existe: la forma pasiva de `5c` es texto con `aria-disabled` y su razón. Lo que **no** existe es de dónde sacar el permiso: `ErpAuthorizationArea.payroll` resuelve a un **único booleano** (`canAccessAccounting`) y es acceso binario **al módulo entero** — o entras a Nóminas o no—. No hay «puede pagar pero no confirmar», ni «sólo lectura», ni permiso por acción. **Inventar esos grados sería inventar dominio**, que es exactamente lo que la compuerta prohíbe: pintaría una celda «sin permiso» que ningún backend puede producir. **Queda abierto hasta que exista una capacidad RBAC real**; cuando exista, la UI ya tiene el vehículo (`blockedReason`) y sólo hay que alimentarlo |
| **5n** matriz de cierre | **parcial** | `5n-p1/p2/p3a/p3b/p4` | n/a | n/a | n/a | n/a | n/a | n/a | sí · **1** en `payroll_redesign_surface_test` | **AUDITADO, no «implementado»: `5n` no es una pantalla.** Es la matriz de 26 transiciones + las notas Flutter + seis decisiones de lógica a aprobar o corregir. **Cumple ya, verificado en código:** control de decisión único con alto 28/44, `ellipsis` sin partir en dos líneas, prohibido chip+botón en la misma celda, anillo de foco de 3 px **fuera** del borde y sin mover layout, `FontFeature.tabularFigures` en todos los estilos mono por el owner de tokens. **Fijado en esta ronda lo que no tenía contrato:** «el `LayoutBuilder` recompone, **nunca desmonta**» — la fila abierta sobrevive a `1440 → 834` sobre el MISMO árbol. Es la invariante que esta migración ya vio fallar. **`maxWidth` 186/168/200 NO está auditado y NO se declara cumplido:** el 186 de escritorio sí está fijado por contrato, pero en tablet el control pide **200 de mínimo** y la columna admite **hasta 280**, con el control a `infinity` — o sea el 200 es un piso, no el máximo que el frame escribe. Falta adjudicar si el frame quiso mínimo o máximo, y con qué razón. **Sólo 1 de las 6 decisiones de lógica está adjudicada** (la nº 5, rechazada). Las otras cinco —horas sin cerrar fuera del total, tolerancia ±$1.000, pagar fuera de orden, «Sin método ▾» sólo por esta semana, y que el anticipo no se aplique solo— **siguen sin revisar una a una**, igual que las **26 filas** de la matriz. `5n` **no se cierra** hasta eso. **«Una superficie, un archivo» NO se cumple y no se declara cumplido:** `payroll_advances_and_cash_surfaces.dart` contiene Anticipos **y** Efectivo. Es una nota de estructura del frame, no un requisito de producto, y se deja como está. **«El `LayoutBuilder` recompone, nunca desmonta» está ADAPTADO, no cumplido:** bajo el breakpoint el host monta otra superficie —`_buildMobile` compone tarjetas—, así que el subárbol sí se desmonta. Lo que importa y sí se cumple es que **el estado de trabajo sobreviva**: vive en el `State` del host. La prueba nueva cubre **la fila abierta**; el borrador del composer y el paso del OCR que la nota también nombra **no** están cubiertos, y del OCR ya está medido que NO sobrevive a salir del módulo. **Divergencias declaradas:** el enum de `5n` es `{paid, transfer, cash, noMethod, locked}` y el nuestro parte en `PayrollRowStatus` + `PayrollRowActionMode`, que separa *qué es* de *cómo se decide* — más estados reales (`nothingToPay`, `weekNotConfirmed`) de los que el frame previó. **Decisión 5 del frame RECHAZADA:** propone el verbo `Imputar` sobre umbrales de confianza 90/70; `Imputar` está **prohibido** por `GUI_DESIGN_PRINCIPLES` (en Chile es acusar de un delito) y los porcentajes de confianza **no existen** en el servicio — es una de las siete afirmaciones falsas que §3 ya documenta. Queda `Aplicar` y las etiquetas sin porcentaje de `7c`. **Sin verificar todavía:** las 26 filas de la matriz una a una |
...
| **OCR paso 2** lectura | **sí** | `5j-paso2.png` | **sí** | **sí** | **sí** | **sí** | **sí** | **sí · vivo** | sí · 9 en `payroll_reconciliation_responsive_test.dart` | **CERRADO CON LAS SEIS CASILLAS (2026-08-02).** **Queda derogado el «no es alcanzable en vivo sin subir un archivo real»**: sí lo es —cargar el archivo es una operación **local**, y el primer write ocurre dentro de `_apply()` (§4.c)—. Verificado contra producción con la cartola real: `page-01.png · 1 página · **14 movimientos**`, la política de cuatro filas visibles y su pie real «… 6 egresos más con todos sus campos reconocidos y sin avisos.», que hasta entonces sólo tenía prueba de harness.  Las capturas de harness anteriores (`shots-final/paso2-*`) quedan como **histórico**, no como evidencia vigente. **Deuda visual exacta y abierta:** separaciones `7`/`10`/`11` del panel, `fromLTRB(17, 9, 17, 11)` del pie y `fontSize 10.5`, todas preexistentes y sin fuente en Design ni owner **834 y 430 CERRADOS EN VIVO el 2026-08-02** contra producción con la cartola real (sesión `89180`, oscuro): `page-01.png · 1 página · 14 movimientos`, `14 movimientos detectados`, ocho filas con su chip (`REVISAR` / `CAMPOS COMPLETOS`), el pie real «… 6 egresos más con todos sus campos reconocidos y sin avisos.», el stepper elidido `Cargar · Lectura · Propuestas · Aplicar` y el CTA `Ver propuestas de pago`. A 834 se lee además `LO QUE LA EXTRACCIÓN AFIRMA` entero: rango `22/07 – 29/07`, las cuatro semanas abiertas, `7 de 14` egresos y `1 línea` que requiere revisión. **Las seis capturas de HARNESS quedan derogadas**, no son evidencia. Capturas: `<scratchpad>/shots-ocr2/paso2-oscuro-{834,430}.png`. **Falta el claro**, que depende del cambio de brillo (ver §4.25) **CLARO CERRADO el 2026-08-02 en las tres anchuras** (1360 / 834 / 430), comprobado por píxel de canvas `#EEF1F5`: `shots-ocr2/paso2-claro-{1360,834,430}.png`. Con eso el paso 2 queda con **las seis casillas** contra producción y datos reales **Evidencia durable** (no sólo scratchpad): `~/Screenshots vinabikeProject/payroll-closure-2026-08-02/` → `01-ocr2-claro-1360.png`, `02-ocr2-claro-834.png`, `03-ocr2-claro-430.png`, y el oscuro de `shots-ocr2/` |
| **OCR paso 4** aplicar | **sí** | `5j-paso4.png` íntegro (104.794 B) | **sí** | **sí** | **sí** | **sí** | **sí** | **sí · vivo** | sí · 9 + navegación/elisión/CTA en `payroll_reconciliation_responsive_test.dart` | **CERRADO FUNCIONALMENTE, CON EVIDENCIA VIVA EN OSCURO (2026-08-01).** Recorrido completo contra **producción** con la cartola real del negocio (`tmp/pdfs/cartola_analysis/page-01.png`, fuera de git): `1 página · 14 movimientos` → **10/10 decisiones** → una sola fila de efectivo respondida (**Semana 30**) → **`Aplicar 1/4 efectivo`**. Cifras reales en pantalla: **`Total a aplicar $38.000`**, **`IMPACTO POR SEMANA · $200.750 → $162.750`**, `Pagos que se crearán 1`, `Excluidos por ti 10`, `Pendiente $38.000 − Anticipo $0 − Efectivo $38.000 = Resto $0`. **TRES celdas vivas, todas OSCURAS: 1360 / 834 / 430** en la misma sesión (`62796`), con el **CTA entero en las tres** —`Confirmar 4 semanas y aplicar conciliación`, sin `Confirmar 4 sema…`— y los pasos sin elidir. Capturas: `<scratchpad>/shots-ocr-final/final-{1360,834,430}.png`. **CLARO NO ESTÁ VERIFICADO EN VIVO PARA ESTA ETAPA**: durante la reconstrucción final no se cambió el brillo. Lo único que existe en claro son las capturas de **HARNESS** del cierre anterior (`<scratchpad>/paso4-bKOxik/paso4-light-{1360,834,430}.png`), con cajas en vez de fuentes — **históricas, sobre un build anterior a las correcciones del CTA y del stepper, y no re-verificadas**. La matriz de seis casillas de este frame **NO está completa**. **`Aplicar` nunca se pulsó** y el borrador se descartó con `Salir sin guardar`. **Deroga la línea anterior de esta fila**, que decía «visualmente de harness» y «no alcanzable en vivo sin subir un archivo real»: eso confundía *subir un archivo* —que es local— con *escribir en producción*; el primer write ocurre dentro de `_apply()` (§4.c). Se conservan las decisiones de contenido: se descartaron `Gasto a Contabilidad`, el desglose «0 nuevos, 4 ya aplicados» y el pie propio del frame; `Total a imputar` → **`Total a aplicar`**. Defectos abiertos: **`wasReplay` está muerto** (§4.22) y **claro sin verificar** **CLARO CERRADO el 2026-08-02, recorrido completo y real.** Se destrabó arreglando la deuda de producto que lo bloqueaba (ver §4.26): las diez preguntas se respondieron re-resolviendo **por identidad de movimiento** en cada vuelta hasta `0 necesitan tu decisión`, se eligió la cuenta ERP `Transferencia · 1110 · Bancos - Cuenta Corriente` y se llegó al paso 4 (`Aplicar 0/4 efectivo`, `Total a aplicar`, y `Confirmar 4 semanas y aplicar conciliación` **deshabilitado**). **`Aplicar` NUNCA se pulsó**; se salió con `Salir sin guardar`. Evidencia **durable y verificada con `find` + `shasum`** en `Screenshots vinabikeProject/payroll-closure-2026-08-02/` (dentro del repo, ignorada por git): `04-ocr4-claro-1360.png` `079cb8fe…`, `05-ocr4-claro-834.png` `1c11c47a…`, `06-ocr4-claro-430.png` `9386111a…` |
...
| Sólo lectura de la hoja de método (5k) | `test/widgets/payroll_method_read_only_authority_test.dart` · 12 · **con control positivo, la unión perfil→autoridad y verificación por mutación** |
...
25. ~~El cambio de brillo no se pudo confirmar desde el agente~~ →
    **RESUELTO el 2026-08-02, y el error era mío.** El brillo SÍ cambia y el
    agente SÍ puede hacerlo: el camino corto es el **quick settings del
    chrome** (`Configuración rápida` → `Tema` → `Claro`), que evita salir del
    módulo. Lo que estaba mal era **dónde muestreaba el píxel**: lo tomaba
    sobre la superficie del menú, que es blanca en claro y en oscuro, así que
    concluí «no cambió» sobre un punto que nunca iba a cambiar.
    **La regla que queda:** el brillo se comprueba sobre el **canvas**
    (`#EEF1F5` claro · `#0D161E` oscuro), nunca sobre una superficie elevada.
    Un píxel mal elegido no es evidencia débil: es evidencia falsa.
