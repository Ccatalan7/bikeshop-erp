...
|---|---|---|---|
| Right toolbar shell | Every desktop workspace, in over-content and reserved-width appearance modes | `_WorkspaceRouterView` hosting `RightToolbar`, plus the stable `_WorkspaceDeepLinkBridge` notification owner in `main.dart` | The toolbar occupies the full workspace height and its collapsed or expanded rail starts at the top edge immediately below the workspace tab bar. Reserved-width mode must stretch the hosting row vertically rather than centering the rail at its intrinsic icon height; long tool lists scroll inside the anchored rail. The stable shell—not each routed `MainLayout`—owns mail, chat and `erp_notifications` lifecycle refresh, Realtime/push listeners, polling fallback and presentation. Existing ERP rows seed a silent baseline; a newly discovered immutable row ID is presented once. Mail, chat and ERP each use a process-wide stable-event gate that fails closed until bound to the exact authenticated user plus tenant. A scope change or shell disposal invalidates the lifecycle epoch, cancels channels/timers, clears shared projections and resets mail resources without closing the shared event stream. Navigating, pushing or resuming several kept-alive routes therefore cannot re-announce handled events or let a late previous-tenant response replace the current feed. |
...
| Purchase receiving | Canonical purchase invoice full page, including OCR-created drafts | `purchase_invoice_form_page.dart` + database purchase receipt kernel | A received parent set expands only through canonical composition links and quantities. Missing or malformed composition fails the receipt atomically; the UI never treats parent header stock as physical inventory. |
...
| Workflow / host | User entry point | Canonical implementation | Required shared behavior |
|---|---|---|---|
| Tenant account removal | `Configuración > Usuarios y roles`, internal and website-account detail panes | `UserManagementPage` + `UserManagementService` + `admin-user-management` | The UI requests a baja and renders the explicit server outcome; it never claims deletion before receiving `auth_deleted`. When any global messaging authorship, participation, attachment, command-receipt or reconciliation evidence references the Auth user, only the requested tenant membership is deactivated and the identity/history remain. Auth is globally banned only when no active staff, customer or worker membership exists in any tenant. The UI reports `deactivated_preserved_messaging_history` distinctly, and deployment order is database migration before Edge Function. |
...
| Daily operational briefing | Right-toolbar bell and the slide-in notifications entry point | `NotificationsToolbarPanel` / `showNotificationsPanel`, both composing `_NotificationBriefing` in `notifications_panel.dart` | Default to the local calendar day with an explicit seven-day comparison; use an open, minimally framed composition with an animated hourly/daily activity pulse, one lightweight metric ribbon and a color-coded timeline; expose one compact icon dropdown for quickly filtering that timeline by jobs, payments, email, chat, Meta interactions, orders, files or other alerts without replacing the mixed default; summarize workshop jobs, payments, online orders and stored files; surface unread mail and chat as attention items; show Meta comments, mentions and other subscribed business interactions as durable `erp_notifications` rows while Meta DMs remain canonical conversations; provider-specific activity rows reuse the corresponding restrained platform glyphs, including the same WhatsApp, Instagram, Messenger, and Web contract as the quick inbox, while Facebook Page interactions use Facebook and aggregate attention rows remain neutral; show a bounded cross-module activity list rather than an unbounded inbox; route a chat activity to `/chat?conversation=<uuid>` and every other summary/activity item through `WorkspaceManager` |
