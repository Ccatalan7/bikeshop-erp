...
| Workflow / host | User entry point | Canonical implementation | Required shared behavior |
|---|---|---|---|
| Routed employee inbox | `/chat` desktop split, compact, and mobile layouts | `EmployeeChatPage` + `ChatProvider` + shared `ConversationTile` + `ChatContextPanel` + `ContextSidePanel` | This is the complete employee inbox for customer, supplier, WhatsApp, web-support, and internal conversations. Search is accent/punctuation-insensitive and applies AND semantics across identity, phone, message and operational context; the active/history control always remains effective while searching. Search and compact menus alter only the projection; they never fork conversation state. Supplier threads with a conversation use the shared row, including provider-backed delivery receipt, time and archival action; only a supplier without a conversation uses the start-chat row. Below 1200 logical pixels of effective content width, operational context starts closed and opens as an overlay instead of compressing the conversation column. Both context readers are responsive and read-only: they present a flat job, bicycle, proposal, sale/boleta, online-order, purchase-invoice or supplier summary and delegate precise record navigation to `WorkspaceManager`; purchases open `/purchases/:id` and suppliers open `/purchases/suppliers/:id/edit`. A parsed `#FV-...` reference may show totals and lines but must never mount `SalesInvoiceEditor`; edits happen only after opening `/sales/invoices/:id`. Outbound requests and mutations remain exclusively in the shared `ChatWindow` and their canonical modules. Conversation previews and unread state load independently from derived job, bicycle, invoice, purchase-invoice, and supplier context. A context refresh remains server-authoritative, preserves the last tenant-and-user-scoped known hint while in flight, coalesces concurrent callers, and notifies only when the rendered context actually changes. |
| Right-toolbar customer messages | Right-toolbar `Mensajería` | `QuickMessagesPanel` + `ChatProvider` + shared `ConversationTile` | A fast customer-focused projection of the canonical inbox, not a second inbox. Opening it may hydrate cached previews and bounded recent message history, then refresh authoritative context without repeating an avoidable full read. Manual reload awaits both. One compact icon menu handles filtering; its selection and active/history scope remain effective during normalized AND search, including whether customer start-chat suggestions are eligible. Permanent chip rows are not a competing navigation model. Opening a thread composes the same `ChatWindow` and capability rules as `/chat`. |
