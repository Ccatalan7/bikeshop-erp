...
accounts = fetch(
    f"accounts?tenant_id=eq.{TENANT}&type=in.(income,expense)&is_active=eq.true"
    "&select=id,code,name,type,category&order=code"
)
...
total_income = round(sum(v['amount'] for v in balances.values() if v['type'] == 'income'), 2)
total_expense = round(sum(v['amount'] for v in balances.values() if v['type'] == 'expense'), 2)
net_income = round(total_income - total_expense, 2)

top_accounts = sorted(
    [v for v in balances.values() if abs(v['amount']) > 0.005],
...
print(json.dumps({
    ...
    'top_income_expense_accounts': top_accounts[:20],
