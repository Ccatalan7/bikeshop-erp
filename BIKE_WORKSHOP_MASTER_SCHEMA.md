...
- Online orders also delegate stock/accounting ownership exclusively to the sales invoice. Unpaid cancellation preserves the order-to-invoice link and records a parent operation connected to the invoice-owned stock/journal reversal; it never records a cash refund. Paid orders must use the invoice Correcciones flow (return, credit note, and separately evidenced refund). Checkout/provider replay controls are preventive; historical paid-order link gaps must not be auto-repaired.
