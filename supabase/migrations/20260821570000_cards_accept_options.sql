-- La base valida las tarjetas con lista cerrada de claves y de tipos, y no
-- conocía `options`, `optionKind` ni la tarjeta `customer_contact`: rechazaba
-- la tarjeta entera y la corrida moría al cerrarse, con
-- `complete_run_v2_rpc_invalid_response`.
--
-- Registrar una tarjeta nueva del asistente son CINCO lugares, no cuatro: la
-- RPC, el contrato del ejecutor, el registry, el allowlist de recibos y esta
-- validación. Ninguno avisa del otro.
...
begin
  ...
  loop
    if jsonb_typeof(v_card) <> 'object'
       ...
       or (v_card ? 'options' and jsonb_typeof(v_card -> 'options') <> 'array')
       or (v_card ? 'optionKind' and jsonb_typeof(v_card -> 'optionKind') <> 'string')
    ...
    if v_kind !~ '^[a-z][a-z0-9_]{0,31}$'
       ...
       or not (
         ...
         or (v_kind in ('conversation','customer_contact')
           and v_destination = 'conversations')
    ...
    if v_card ? 'entityRef' then
      ...
      if v_kind in (
         ...
         or not (
           ...
           or (v_kind = 'customer_contact' and v_entity_ref ->> 'kind' = 'customer')
         ) then return false; end if;
    end if;
    -- Las opciones son excluyentes y cerradas: id, rótulo y la revisión que se
    -- mostrará antes de confirmar. Elegir una nunca ejecuta nada, así que aquí
    -- no viaja ninguna referencia ni acción: sólo texto que el operador leerá.
    if v_card ? 'options' then
      if jsonb_array_length(v_card -> 'options') > 6
         or coalesce(octet_length(v_card ->> 'optionKind'), 0) not between 1 and 40
         or exists (
           select 1 from jsonb_array_elements(v_card -> 'options') option
           where jsonb_typeof(option) <> 'object'
             or not (option ? 'id' and option ? 'label')
             or jsonb_typeof(option -> 'id') <> 'string'
             or jsonb_typeof(option -> 'label') <> 'string'
             or (option ? 'description'
               and jsonb_typeof(option -> 'description') <> 'string')
             or octet_length(option ->> 'id') not between 1 and 64
             or octet_length(option ->> 'label') not between 1 and 80
             or coalesce(octet_length(option ->> 'description'), 0) > 600
             or exists (select 1 from jsonb_object_keys(option) key
               where key not in ('id','label','description'))
         ) then return false; end if;
    elsif v_card ? 'optionKind' then
