-- Recover inference latency after template binding without relaxing definition identity.
-- Measured candidate SELECT on production: KMC 836.558 ms; previous RPC >30 s.
-- Only this private reader changes. No product/spec data or runtime mutation.
begin;
do $$
declare v_oid oid:=to_regprocedure('public.assistant_infer_technical_predicates_internal_v1(uuid,text)');
begin
 if v_oid is null or md5(pg_get_functiondef(v_oid)) not in ('81c8b719dd1045f2a9c5344a47920ea4','aa5c3a35b1a76c3b280a34838b6b63f9') then
  raise exception 'Inference reader changed since the reviewed performance preimage';
 end if;
 if (select proacl::text from pg_proc where oid=v_oid) is distinct from '{postgres=X/postgres,service_role=X/postgres}' then
  raise exception 'Inference reader ACL changed since review';
 end if;
end $$;

-- Each eligible definition is resolved once per template and tenant. A key
-- with multiple IDs remains unusable. Active facts then join by the exact ID,
-- tenant, subject and effective binding, rather than invoking three nested
-- scalar readers for every observation. An invalid explicit binding never
-- falls back to a category. The three inference consumers reuse this set.
...
with params as (
  ...
  ), active_definitions as materialized (
    select tf.template_id,d.key,(array_agg(distinct d.id))[1] definition_id
    from params p cross join public.spec_templates t
    join public.spec_template_fields tf on tf.template_id=t.id
    join public.spec_definitions d on d.id=tf.spec_definition_id
    where t.is_active and (t.tenant_id is null or t.tenant_id=p.tid)
      and (d.tenant_id is null or d.tenant_id=p.tid)
      and coalesce(t.form_contract->'roles'->>d.key,'primary')<>'legacy'
    group by tf.template_id,d.key having count(distinct d.id)=1
  ), active_facts as materialized (
    select f.tenant_id,f.subject_type,f.subject_scope,f.subject_id,f.spec_definition_id,f.value_number
    from params p join public.spec_facts f on f.tenant_id=p.tid
      and f.subject_type='product' and f.subject_scope is null
    join public.products product on product.id=f.subject_id and product.tenant_id=f.tenant_id
    left join public.category_tech_mappings m on m.tenant_id=product.tenant_id
      and m.category_id=product.category_id and m.status='active' and product.spec_template_id is null
    join active_definitions a on a.template_id=case when product.spec_template_id is not null then product.spec_template_id else m.template_id end
      and a.definition_id=f.spec_definition_id
  ), scoped as (
...
-- CREATE OR REPLACE preserves the reviewed owner and ACL; assert both.
do $$
begin
 if md5(pg_get_functiondef('public.assistant_infer_technical_predicates_internal_v1(uuid,text)'::regprocedure)) <> 'aa5c3a35b1a76c3b280a34838b6b63f9' then
  raise exception 'Inference reader postimage differs';
 end if;
 if (select proacl::text from pg_proc where oid='public.assistant_infer_technical_predicates_internal_v1(uuid,text)'::regprocedure) is distinct from '{postgres=X/postgres,service_role=X/postgres}'
   or (select pg_get_userbyid(proowner) from pg_proc where oid='public.assistant_infer_technical_predicates_internal_v1(uuid,text)'::regprocedure)<>'postgres' then
  raise exception 'Inference reader authority differs';
 end if;
end $$;
commit;
