-- Una medida pegada a su unidad tiene que leerse como medida.
--
-- El asistente no resolvía «48mm» en ninguna redacción. Con `48 mm` separado sí,
-- pero nadie escribe así: el catálogo mismo dice «48MM». El tokenizador corta
-- por caracteres no alfanuméricos, así que «48mm» quedaba de una pieza y nunca
-- igualaba al rótulo «48».
--
-- Ahora un número pegado a un sufijo de letras entrega **también** el número
-- suelto, con la misma posición, para que la adyacencia siga viendo un solo
-- token. Entra «700c», «2.1in», «60mm». Queda fuera «26x1.95» a propósito: eso
-- es un calce de neumático y partirlo inventaría un valor que nadie pidió.
--
-- Procesar números y medidas no es un extra en este negocio: casi todo lo que
-- el taller pide es una medida.
