-- Una coincidencia ampliada lo dice.
--
-- La ficha rotula «COINCIDE CON “CÁMARAS 29 CON VÁLVULA SCHRADER”» y debajo
-- aparecía una cámara **V/FRANCESA** —válvula Presta, que es justo la otra—.
-- No es un error del resolutor: cuando no hay suficientes resultados suelta la
-- medida menos determinante, y ahí soltó la válvula. El error es del rótulo,
-- que prometía una coincidencia exacta sobre un resultado ampliado.
--
-- El servidor ya sabe qué soltó. Se publica, y el rótulo lo dice.

begin;

drop function if exists public.supplier_catalog_page_v1(uuid, text, integer, integer, text);
