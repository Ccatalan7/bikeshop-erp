-- La traducción de la frase dependía de que el modelo no mandara predicados.
-- Eso deja la decisión de «entender o no» en manos del modelo: si arma un
-- predicado pobre, el filtro de texto vuelve a exigir cada palabra en el nombre
-- y la respuesta vuelve a ser cero. Ahora traduce siempre y sólo cede el campo
-- que el modelo ya constriñó.
--
-- Medido el 2026-08-21: la inferencia cuesta menos de 1 ms; correrla en cada
-- búsqueda no compromete el `statement_timeout` de 4500 ms.
...
begin
  ...
  if v_query is not null then
    ...
    into v_inferred_predicates
    from jsonb_array_elements(v_inferred -> 'predicates') candidate(value)
    where not exists (
      select 1
      from jsonb_array_elements(p_technical_predicates) existing(value)
      where existing.value ->> 'field' = candidate.value ->> 'field'
    );
    if jsonb_array_length(v_inferred_predicates) > 0 then
      p_technical_predicates := p_technical_predicates || v_inferred_predicates;
