alter table public.stock_adjustments
  add column if not exists adjustment_origin text;

create index if not exists idx_stock_adjustments_tenant_origin
  on public.stock_adjustments(tenant_id, adjustment_origin);
...
alter view public.stock_movements_view set (security_invoker = on);
...
begin
  ...
  case p_reason_type
    when 'count' then
      ...
      v_reason_label := 'Regularizacion por conteo';
    when 'loss' then
    ...
    if p_type = 'OUT' then
      case p_reason_type
        ...
        when 'damage' then
          ...
          v_counterpart_account_name := 'Perdidas por Dano de Inventario';
        when 'theft' then
          v_counterpart_account_code := '6197';
          v_counterpart_account_name := 'Perdidas por Robo de Inventario';
        when 'loss' then
