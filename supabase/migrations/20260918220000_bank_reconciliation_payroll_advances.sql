-- Advances in the bank reconciliation.
--
-- The owner pays part of a week in cash during it (Braulio, week 34: a
-- $30.000 cash advance on 20 August, then $10.600 by transfer). Nómina had the
-- advance; the review did not, so the $10.600 transfer matched no salary and
-- the answer recorded for it paid the $30.000 again as cash: the cash left
-- the till twice in the books until it was reversed.
--
--   * The candidate catalog v2 returns `open_advances` (worker, available
--     amount, business date paid, method), so the review expects the week's
--     balance minus the advances paid on or before its end.
--   * apply_bank_reconciliation_actions_v3 accepts `advances` on a
--     pay_payroll row and applies them in the same pay_payroll_voucher_v2
--     call, with the transfer as the payment. It checks what the kernel
--     checks — the worker's own, still open, enough left, paid by the week's
--     end — and that Nómina returned one allocation per advance.
--
-- Both functions keep their signatures.
...
comment on function public.apply_bank_reconciliation_actions_v3(
  uuid, bigint, text, jsonb
) is 'v2 plus pay_payroll: pays an owed Nómina line through confirm_payroll_voucher_v2/pay_payroll_voucher_v2 on the bank date, applying the advances the row names, and associates the row with that payment, atomically.';
