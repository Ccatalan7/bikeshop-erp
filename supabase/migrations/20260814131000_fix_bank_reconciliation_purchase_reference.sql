-- Bank reconciliation purchase-payment reference compatibility.
--
...
-- this task.
--
-- The foundation accidentally read `invoice_reference` from
-- purchase_payments. That field exists only on sales_payments; purchase
-- payments own `reference`. Replacing the sealed snapshot helper fixes the
-- candidate projection without changing or backfilling any business row.
...
comment on function public.bank_reconciliation_target_snapshot(
  uuid, uuid, text, uuid
) is 'Sealed bank-reconciliation target projection. Purchase references come from purchase_payments.reference.';
