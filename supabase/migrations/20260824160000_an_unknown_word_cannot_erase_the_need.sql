-- Una palabra que el vocabulario no conoce no puede borrar la necesidad.
--
-- Medido con frases reales del taller, sobre datos de producción:
--
--   «aros 26»          → 0 proveedores   (teniendo Llantas con historial)
--   «platos y bielas»  → 0 proveedores   (teniendo Transmisión con historial)
--   «rayos 27.5»       → 4 proveedores   ✓
--
-- La diferencia no es el dato: es que «aros» y «platos» no son las palabras con
-- las que bautizamos las categorías. El filtro de texto exige que CADA palabra
-- del residuo esté en el nombre del producto, así que una sola palabra ajena
-- mataba la consulta entera.
--
-- Ahora hay una segunda pasada sin el residuo, y las filas dicen que la hubo
-- (`scopeRelaxed`, `droppedWords`) para que la respuesta no presente como
-- literal algo que se ensanchó. La pasada sólo existe cuando la rama o la
-- medida YA acotaron la pregunta: sin nada resuelto, soltar el texto
-- significaría «a quién le compramos todo», que no es una pregunta del taller.
--
-- Es el mismo principio que gobierna el resto del módulo: se repara la forma y
-- se rechaza el vocabulario, nunca al revés.
...
begin
  ...
  -- **Una palabra que el vocabulario no conoce no puede borrar la necesidad.**
  --
  -- El filtro de texto exige que CADA palabra del residuo esté en el nombre
  -- del producto. Medido con frases reales del taller: «aros 26» y «platos y
  -- bielas» devolvían CERO proveedores teniendo historial, porque «aros» y
  -- «platos» no son las palabras del catálogo —ahí se llaman «Llantas» y viven
  -- bajo Transmisión—. El operador no tiene por qué saber cómo bautizamos las
  -- categorías.
  --
  -- Segunda pasada sin el residuo, y sólo entonces: la rama o la medida ya
  -- acotaron la pregunta, así que soltar el texto la ensancha, no la borra.
  -- Si NADA se resolvió, no hay segunda pasada: sin categoría ni predicado,
  -- soltar el texto significaría «a quién le compramos todo».
