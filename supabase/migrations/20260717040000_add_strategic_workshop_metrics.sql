-- Strategic workshop dashboard: honest operational KPIs plus the one missing
-- input required for revenue-per-labor-hour. Historical rows stay null so the
-- dashboard can expose coverage instead of fabricating data.
begin;
...
alter table public.mechanic_jobs
  drop constraint if exists mechanic_jobs_actual_labor_hours_valid;
alter table public.mechanic_jobs
  add constraint mechanic_jobs_actual_labor_hours_valid
  check (
    actual_labor_hours is null
    or actual_labor_hours between 0 and 10000
  );

comment on column public.mechanic_jobs.actual_labor_hours is
  'Manual total of real technician labor hours spent on the job. Operational only; it does not post payroll, revenue, tax, accounting, or inventory.';
  ...
  return (
    ...
    classified_sales as (
      select
        ...
        case
          when lower(coalesce(item ->> 'is_service', 'false')) in ('true', 't', '1')
            or lower(coalesce(item ->> 'item_type', 'product')) = 'service'
            then 'service'
          else 'product'
        end as sale_kind,
    ...
    select jsonb_build_object(
      ...
      'value', jsonb_build_object(
        ...
        'serviceSalesShare', (
          select case when service_sales + product_sales <= 0 then null
            else service_sales / (service_sales + product_sales) end
          from sales_mix
        ),
        'productSalesShare', (
          select case when service_sales + product_sales <= 0 then null
            else product_sales / (service_sales + product_sales) end
          from sales_mix
        ),
...
comment on function public.get_strategic_dashboard_metrics(timestamp with time zone, timestamp with time zone) is
  'Tenant-scoped strategic workshop and inventory metrics. Every ratio includes its sample or coverage; labor productivity uses only explicitly recorded actual_labor_hours.';
