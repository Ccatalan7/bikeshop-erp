-- Un motor, dos puertas: el asistente y el módulo preguntan lo mismo.
--
-- El paso «Proveedores» del Asistente de compras respondía POR PRODUCTO. Para
-- «neumáticos 27.5 económicos» listaba un 26x1.75, un 29x2.25 y un 700x28C
-- —medidas que no son la pedida— cada uno rotulado «Sin verificar · flete sin
-- clasificar · Sin banda aún». Visto en la app real, no en el JSON.
--
-- La pregunta del taller es otra: a QUIÉN le compramos esto. Ese análisis ya
-- existe, pero vivía tras la capacidad `ai.read.purchases`, que es del
-- asistente y no de un operador cualquiera en el módulo de compras.
--
-- Se separa el motor del portero, igual que ya conviven
-- `rank_purchase_candidates_v1` (módulo) y su gemela `assistant_…` (IA):
--
--   purchase_supplier_concentration_internal_v1  ← el análisis, una sola vez
--   assistant_rank_purchase_suppliers_v1         ← puerta IA, con capacidad
--   rank_purchase_suppliers_v1                   ← puerta del módulo, por tenant
--
-- Duplicar el análisis en dos funciones habría dejado dos verdades que se
-- separan en el primer arreglo que toque sólo una.
...
-- La puerta del asistente: exige la capacidad de IA y devuelve el sobre que el
-- gateway sabe validar.
...
begin
  ...
  v_analysis := public.purchase_supplier_concentration_internal_v1(
    v_authority.tenant_id, p_query, p_category, p_brand, p_limit
  );
...
-- La puerta del módulo: el mismo análisis para el operador que ya está dentro
-- del Asistente de compras, sin pedirle una capacidad del asistente de IA.
create or replace function public.rank_purchase_suppliers_v1(
  p_query text default null,
  p_category text default null,
  p_brand text default null,
  p_limit integer default 5
...
begin
  ...
  v_analysis := public.purchase_supplier_concentration_internal_v1(
    v_tenant_id, p_query, p_category, p_brand, p_limit
  );
...
revoke all on function public.rank_purchase_suppliers_v1(
  text, text, text, integer
) from public, anon, authenticated, service_role;
grant execute on function public.rank_purchase_suppliers_v1(
  text, text, text, integer
) to authenticated;
