-- Palabra completa, no trozo de palabra.
--
-- El escalón final enganchaba por subcadena, y eso devolvía respuestas de otro
-- producto con toda seguridad. Medido: «MAXXIS ALAMBRE 29X2.20 ARDENT RACE»
-- traía «RaceLub» —un lubricante— porque «race» está dentro de «RaceLub».
--
-- Con palabra completa entra además «aro», de tres letras, que es justo la
-- palabra con la que el taller nombra la medida: «aros 26» funcionaba sólo en
-- plural, porque el corte por largo dejaba fuera el singular. Que una respuesta
-- dependa del número gramatical es el mismo defecto de antes, al revés.
        ...
        and (
          ...
          or (
            case when v_match_any then
              -- **Palabra completa, no trozo.** Por subcadena, «race» de
              -- «ARDENT RACE» enganchaba dentro de «RaceLub» y el análisis
              -- devolvía un lubricante como si fuera un neumático. Con palabra
              -- completa también entra «aro», que tiene tres letras y es la
              -- palabra con la que el taller nombra la medida.
              exists (
                select 1 from regexp_split_to_table(v_query, ' +') token
                where length(token) >= 3
                  and (
                    ' ' || purchased.identity_surface || ' '
                      like '% ' || token || ' %'
                    or ' ' || purchased.identity_surface || ' '
