-- El contrato de recibos anuncia `rank_purchase_suppliers`.
--
-- Una herramienta que la base no anuncia se ejecuta y se descarta después:
-- el recibo no encuentra su contrato y el resultado no llega al modelo. Ya
-- pasó con la búsqueda pública —«fuente no disponible» con la búsqueda
-- ejecutada—, así que el registro va en la misma tarea que la RPC.
--
-- Tope de 5: la respuesta útil es «a estos dos o tres les compramos esto»,
-- no la lista completa de proveedores del taller.
