-- La tarjeta de borrador aprende el objetivo comercial que la IA elige.
--
-- **Primer corte del objetivo comercial por IA, y a propósito el primero.** La
-- fase A ya enseñó cuánto cuesta el orden inverso: amplió la línea del borrador
-- con la procedencia de categoría, no migró `assistant_cards_valid_v1`, y la
-- vía conversacional quedó rota sin que el síntoma se pareciera a la causa
-- —las seis herramientas en `succeeded` y un 500 genérico—. Por eso el
-- validador se amplía **antes** de que nada emita la clave: cuando el gateway
-- empiece a mandarla, la base ya sabe aceptarla.
--
-- **El dominio es el de `supply_need_commercial_revisions`, no uno nuevo.** Un
-- validador más permisivo que la tabla deja pasar tarjetas que el comando
-- después rechaza, y el operador ve fallar algo que la interfaz le mostró como
-- válido. Gama en su enum, marca como identidad UUID, techo de costo con el
-- `between` que además excluye `NaN` e `Infinity` —que `numeric` acepta y son
-- ambos mayores que cero—, y margen en 0..1.
--
-- **La moneda no es representable.** `set_supply_need_commercial_target_v1` la
-- fija desde `tenants.currency` y rechaza una carga que la traiga; una tarjeta
-- que la incluya se rechaza igual, en vez de ignorarla en silencio.
--
-- **Opcional, como la procedencia de categoría.** Las tarjetas ya persistidas
-- no la traen y exigirla invalidaría filas vivas. Un objeto vacío tampoco vale:
-- sería ruido que el comando tendría que distinguir de «no se pidió nada».
--
      ...
      loop
        if jsonb_typeof(v_supply_line) <> 'object'
           ...
           or exists (select 1 from jsonb_object_keys(v_supply_line) key where key not in (
             ...
             -- Objetivo comercial elegido por la IA. Opcional, como las de
             -- arriba: las tarjetas anteriores no lo traen.
             'commercialTarget'
           ))
           or (v_supply_line ? 'commercialTarget' and not (
             jsonb_typeof(v_supply_line -> 'commercialTarget') = 'null'
             or (
               jsonb_typeof(v_supply_line -> 'commercialTarget') = 'object'
               -- Dominio cerrado, identico al de
               -- `supply_need_commercial_revisions`: un validador que admita
               -- mas que la tabla deja pasar tarjetas que el comando despues
               -- rechaza, y el operador ve fallar algo que la interfaz mostro
               -- como valido.
               and not exists (
                 select 1
                 from jsonb_object_keys(v_supply_line -> 'commercialTarget') key
                 where key not in (
                   'gama','preferredBrandId','maxLandedUnitCostNet',
                   'minGrossMarginRatio'
                 )
               )
               -- La moneda es server-owned y **no es representable**: una
               -- tarjeta que la traiga se rechaza, no se ignora.
               and not (v_supply_line -> 'commercialTarget' ? 'currencyCode')
               and (
                 not (v_supply_line -> 'commercialTarget' ? 'gama')
                 or (v_supply_line -> 'commercialTarget' ->> 'gama')
                   in ('economica','media','alta')
               )
               and (
                 not (v_supply_line -> 'commercialTarget' ? 'preferredBrandId')
                 or lower(
                   v_supply_line -> 'commercialTarget' ->> 'preferredBrandId'
                 ) ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
               )
               and (
                 not (v_supply_line -> 'commercialTarget' ? 'maxLandedUnitCostNet')
                 or (
                   jsonb_typeof(
                     v_supply_line -> 'commercialTarget' -> 'maxLandedUnitCostNet'
                   ) = 'number'
                   -- `between` deja fuera `NaN` e `Infinity`, que numeric
                   -- acepta y los dos son mayores que cero.
                   and (v_supply_line -> 'commercialTarget'
                     ->> 'maxLandedUnitCostNet')::numeric
                     between 0.0001 and 999999999
                 )
               )
               and (
                 not (v_supply_line -> 'commercialTarget' ? 'minGrossMarginRatio')
                 or (
                   jsonb_typeof(
                     v_supply_line -> 'commercialTarget' -> 'minGrossMarginRatio'
                   ) = 'number'
                   and (v_supply_line -> 'commercialTarget'
                     ->> 'minGrossMarginRatio')::numeric between 0 and 1
                 )
               )
               -- Un objetivo vacio no es un objetivo: seria ruido que el
               -- comando tendria que distinguir de «no se pidio nada».
               and (v_supply_line -> 'commercialTarget') <> '{}'::jsonb
             )
           ))
...
comment on function public.assistant_cards_valid_v1(jsonb) is
  'Closed validator for persisted assistant cards. A supply-need draft line may carry category provenance (phase A) and an optional commercialTarget whose domain mirrors supply_need_commercial_revisions exactly: gama enum, brand identity, landed-cost ceiling excluding NaN and Infinity, margin floor in 0..1. currencyCode is server-owned and unrepresentable, and an empty target object is rejected rather than treated as absent.';
