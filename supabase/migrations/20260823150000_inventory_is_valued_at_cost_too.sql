-- El inventario también se valoriza a costo.
--
-- «¿Cuánto tengo en inventario?» tenía una sola respuesta —el valor a precio de
-- venta— y son dos preguntas distintas: a precio de venta es lo que vale si se
-- vende todo; a costo es la plata que el taller tiene inmovilizada, que es la
-- que importa para saber cuánto se está financiando el stock. Medido hoy en el
-- taller: $12,3 M a precio de venta contra $5,4 M a costo.
--
-- `inventoryCostValue` suma sólo lo que tiene costo cargado y `costedCount`
-- dice sobre cuántos productos se calculó, para que la respuesta pueda decir
-- «sobre 1.520 de 1.612» en vez de dar un total que parece completo y no lo es.
