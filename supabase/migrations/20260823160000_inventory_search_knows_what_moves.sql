-- El buscador sabe cuánto se movió cada producto.
--
-- «¿Qué tengo en stock que nunca se ha vendido?» —stock muerto, plata parada en
-- la estantería— caía en «no tengo una herramienta autorizada». El asistente
-- incluso buscaba «nunca vendidos» como texto, que no significa nada.
--
-- Se reutiliza EXACTAMENTE la misma definición de demanda que usa la
-- reposición: unidades vendidas más consumidas en taller en 90 días. Dos
-- definiciones distintas de «se movió» darían dos respuestas distintas a la
-- misma pregunta.
--
-- `soldRecently` viaja en cada fila y además se puede filtrar y ordenar como
-- predicado operacional (`sold_recently`), así que «nunca se vendió» es
-- `sold_recently = 0` y no una frase que el modelo tenga que interpretar.
