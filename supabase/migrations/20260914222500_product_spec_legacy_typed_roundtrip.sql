...
lock table public.products,public.spec_facts,public.spec_fact_values,public.spec_fact_readings,public.product_spec_values,public.product_spec_member_profiles,public.product_spec_member_profile_events,public.product_spec_save_receipts in share row exclusive mode;
create temp table legacy_roundtrip_before on commit drop as select jsonb_build_object('products',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.products x),'spec_facts',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.spec_facts x),'spec_fact_values',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.spec_fact_values x),'spec_fact_readings',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.spec_fact_readings x),'product_spec_values',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_values x),'product_spec_member_profiles',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_member_profiles x),'product_spec_member_profile_events',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_member_profile_events x),'product_spec_save_receipts',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_save_receipts x)) value;
...
do $data$ begin
 if (select value from legacy_roundtrip_before) is distinct from jsonb_build_object('products',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.products x),'spec_facts',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.spec_facts x),'spec_fact_values',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.spec_fact_values x),'spec_fact_readings',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.spec_fact_readings x),'product_spec_values',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_values x),'product_spec_member_profiles',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_member_profiles x),'product_spec_member_profile_events',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_member_profile_events x),'product_spec_save_receipts',(select md5(coalesce(jsonb_agg(to_jsonb(x) order by to_jsonb(x)::text),'[]')::text) from public.product_spec_save_receipts x)) then
