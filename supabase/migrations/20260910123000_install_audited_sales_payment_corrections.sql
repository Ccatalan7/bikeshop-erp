-- Installs the audited sales-payment correction command in production.
--
-- Cause. 20260723023000_add_audited_sales_payment_corrections.sql was never
-- deployed (stamp NOT_APPLIED), while the client that calls
-- correct_sales_payment shipped on 2026-07-24. 20260819180000 redefined the
-- command with the per-tender tax rule but, by design, skipped the replacement
-- where the July event table was absent — i.e. in production. Since then the
-- payment detail form answers every correction with PGRST202 ("Could not find
-- the function public.correct_sales_payment"); an operator reported it on
-- 2026-09-10.
--
-- Why this file and not the July one. The July migration also redefines
-- validate_sales_payment_integrity() with its July body, which pins the payment
-- to the invoice's tax treatment. Deploying it now would regress the
-- per-tender classification that 20260819180000 installed. This file leaves
-- validate_sales_payment_integrity() untouched, asserts up front that it
-- already carries both the correction-command hook and the tender-tax rule,
-- and installs correct_sales_payment with the 2026-08-19 body.
--
-- Forward: sales_payment_edit_events (immutable receipts + before/after
-- evidence, tenant-scoped RLS), the correction-command guard trigger on
-- sales_payments, get_sales_payment_edit_operation(text) for lost-ack replay,
-- correct_sales_payment(...) and their ACLs. No data rewrite, no backfill.
-- Idempotent: every object is created with if-not-exists / or-replace /
-- drop-if-exists.
--
-- Lock/timeout: the trigger on public.sales_payments takes a short
-- SHARE ROW EXCLUSIVE lock; lock_timeout 5s makes a busy table fail fast so the
-- same file is simply rerun.
--
-- Recovery: drop trigger trg_sales_payments_01_correction_command on
-- public.sales_payments and revoke the two RPC grants from authenticated.
-- Preserve the event table and its accounting evidence; never drop history.
--
-- Constraint carried from the July design: with an employee session, a
-- sales_payments row can only change payment_method_id/amount/date/reference/
-- notes through the audited command, and deleted_at/deleted_by never change
-- directly. That includes the cascade in handle_sales_invoice_change when a
-- posted invoice with active payments is reverted; the client only offers that
-- reversal when paid_amount = 0. Service-role/provider flows (auth.uid() null)
-- are unaffected.
begin;
...
declare
  v_validate text;
begin
  if to_regprocedure('public.validate_sales_payment_integrity()') is null then
    raise exception 'validate_sales_payment_integrity() is missing';
  end if;
  v_validate := pg_get_functiondef(
    'public.validate_sales_payment_integrity()'::regprocedure
  );
  if v_validate not like '%app.sales_payment_correction_command%' then
    raise exception
      'validate_sales_payment_integrity() lacks the correction-command hook';
  end if;
  if v_validate not like '%Tax follows the tender%' then
    raise exception
      'validate_sales_payment_integrity() lacks the 2026-08-19 tender-tax rule';
  end if;
  if (
    select count(*)
     ...
     where table_schema = 'public'
       ...
       and column_name in (
         'invoice_reference', 'idempotency_key', 'tax_treatment',
         'net_amount', 'iva_amount', 'deleted_at', 'deleted_by'
       )
  ) <> 7 then
    raise exception 'sales_payments lacks a column the correction guard reads';
  end if;
end;
$pre$;
