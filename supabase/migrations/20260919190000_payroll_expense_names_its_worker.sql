-- Un gasto de sueldo dice a quién se le pagó.
--
-- El dueño abrió el Resumen diario y encontró «GTO-00196 · Proveedor no
-- informado · $10.000» una y otra vez: 109 de sus 195 gastos son sueldos, y
-- un sueldo no tiene proveedor. La contraparte es el trabajador, y el ERP la
-- sabía —la escribía en las notas y en la línea del gasto— pero dejaba vacío
-- el único campo que leen la lista de gastos, la ficha, el panorama y la
-- notificación, que entonces dicen «Proveedor no informado» / «Proveedor sin
-- nombre», como si faltara el dato.
--
-- `supplier_name` es en este ERP el nombre libre de la contraparte cuando no
-- hay ficha de proveedor —así lo escribe ya la conciliación bancaria para
-- Google, Meta o el arriendo—, así que el sueldo escribe ahí al trabajador y
-- el extra pagado a una persona, a su beneficiario. `supplier_id` sigue nulo:
-- nadie los convierte en proveedores, y las guardias de procedencia que
-- miran `supplier_id` no cambian.
--
...
begin
  ...
  insert into public.expenses (
    ...
    payment_status, supplier_name, created_by
  ) values (
    ...
    -- A salary has no supplier: whoever received the money is the worker,
    -- and the expense names him where every surface reads the counterparty.
    'posted', 'pending', v_line.employee_name, auth.uid()
...
CREATE OR REPLACE FUNCTION public.apply_payroll_payment_workspace_v1(p_workspace_id uuid, p_operation_key text, p_expected_workspace_version bigint, p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  loop
    ...
    insert into public.expenses (
      ...
      payment_status, reference, notes, supplier_name, created_by
    ) values (
      ...
      -- Same for an extra paid to a worker: the beneficiary is who received
      -- it, never a supplier.
      (
        select nullif(btrim(
        ...
        where employee.id = concept_element.beneficiary_employee_id
          and employee.tenant_id = tenant_id_value
      ),
      actor_id_value
...
-- Lo ya escrito se completa desde su propia evidencia: la línea de la semana
-- para un sueldo, el beneficiario del concepto para un extra. Sólo se llena
-- lo que está vacío; nada que el operador haya escrito se pisa.
do $$
declare
  v_sueldos integer := 0;
  v_extras integer := 0;
...
begin
  ...
  with nombrados as (
    update public.expenses expense
       set supplier_name = line.employee_name,
           updated_at = now()
  ...
  select count(*) into v_sueldos from nombrados;

  with nombrados as (
    update public.expenses expense
       set supplier_name = line.employee_name,
           updated_at = now()
      from public.payroll_payment_workspace_concept_dispositions concept
      join public.payroll_voucher_lines line
        on line.id = concept.voucher_line_id
       and line.tenant_id = concept.tenant_id
     where concept.result_expense_id = expense.id
       and concept.tenant_id = expense.tenant_id
       and expense.supplier_id is null
       and coalesce(btrim(expense.supplier_name), '') = ''
       and coalesce(btrim(line.employee_name), '') <> ''
    returning expense.id
  ...
  -- El aviso que el dueño ya tiene en la bandeja también decía «Proveedor no
  -- informado»: se reescribe con el nombre, que es lo que el gasto dice ahora.
  with corregidos as (
  ...
  if v_pendientes > 0 then
    raise exception 'payroll_expense_counterparty_backfill_incomplete: %',
      v_pendientes;
  end if;

  raise notice 'contrapartes escritas: % sueldos, % extras, % avisos',
    v_sueldos, v_extras, v_avisos;
end;
$$;
