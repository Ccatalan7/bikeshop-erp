-- «Sin banda 7» no es un hallazgo, es la ausencia del dato.
--
-- La tarjeta de Derman salía en la app real con «Costo unitario promedio
-- $7.810 • Última compra hace 5 meses • sin banda 7». Los primeros dos datos
-- deciden una compra; el tercero es ruido que ocupa el mismo renglón y le
-- resta autoridad a los otros dos.
--
-- Cuando ninguna línea del proveedor tiene banda derivada, la mezcla se calla.
-- Si al menos una la tiene, se muestra entera —incluido cuántas quedaron sin
-- banda—, porque ahí sí es una salvedad sobre un dato que existe.
--
-- Visto en el frame, no en el JSON: el resultado era correcto y se leía mal.
