-- El riesgo de inventario deja de ser «filas donde stock <= mínimo» y pasa a
-- ser «qué hay que reponer».
--
-- Medido en producción el 2026-08-22 sobre el tenant real: la definición
-- anterior marcaba 1.016 productos en riesgo, de los cuales 905 estaban
-- simplemente en cero, y sólo 173 productos se movieron en 90 días. El tope de
-- 10 filas se llenaba con catálogo que nunca rotó, así que la respuesta era
-- exacta por fila e inservible como respuesta: el dueño no podía actuar sobre
-- ninguna de las diez.
--
-- Dos cambios, y ninguno inventa un número:
--   * el orden lo manda la demanda real —lo vendido y lo consumido en taller en
--     los últimos 90 días—, y recién dentro de eso la severidad;
--   * el sobre dice cuántas coincidencias hay en total, no cuántas cupieron.
--     «Te muestro 10» sin «de 1.016» suena a lista completa.
--
-- `suggestedOrder` es aritmética explicable, no un pronóstico: lo que falta
-- para llegar al mínimo, o un mes de la demanda observada, lo que sea mayor.
...
as $function$
  select jsonb_build_object(
    ...
    -- Nunca menor que lo devuelto: el validador de la función lo exige, y un
    -- total menor que la página sería una contradicción visible para el modelo.
    'totalMatches', greatest(
      coalesce(p_total_matches, jsonb_array_length(coalesce(p_items, '[]'::jsonb))),
...
begin
  ...
  with demanda as materialized (
    ...
    select matched.*,
      row_number() over (order by
        -- Lo que rota manda. Un producto en cero que nunca se vendió es
        -- catálogo, no una urgencia: iba primero y desplazaba a los reales.
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'supplierName', nullif(public.assistant_truncate_utf8_internal_v1(
        coalesce(supplier_name, ''), 120), '')
    ) order by ordinal) filter (where ordinal <= p_limit), '[]'::jsonb),
    coalesce(max(total_matches), 0)
