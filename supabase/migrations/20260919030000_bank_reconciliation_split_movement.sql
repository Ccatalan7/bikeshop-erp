-- Split one bank movement across several accounts.
--
-- The owner's mother pays things for the shop — the accountant's fee
-- (Pedro Madrid), the monthly F29 and the municipal licence — and he repays
-- her in one transfer: $214.685 on 17 August were F29 $140.149 and the second
-- half-year licence $74.536. The review could only book a movement to one
-- account, so those transfers matched nothing.
--
--   * The action kernel accepts `split`: 2 to 10 parts that add up to the
--     movement. A part on an expense account becomes an expense paid from the
--     bank, with its supplier, exactly as `create_expense` books one; any
--     other account is a line of one balanced journal against the bank.
--   * The candidate catalog keeps the parts in `decisions`, so the next
--     transfer of the same kind can be proposed with the same structure.
--
...
comment on function
  ...
  'Applies the open rows of one bank-statement review atomically. Decided rows are final; a movement another statement of the account already settled can only be dismissed as such. A split books one movement as several expenses and one journal. Generated expenses and journals are posted before their immutable evidence allocation is persisted.';
  ...
  select coalesce(jsonb_agg(item order by item->>'decided_at' desc), '[]'::jsonb)
    ...
    from (
      select jsonb_build_object(
        ...
        'parts', decision.action_snapshot->'parts',
       ...
       where decision.tenant_id = v_tenant_id
         and decision.action_kind in (
           'create_expense', 'post_journal', 'dismiss', 'split'
         )
