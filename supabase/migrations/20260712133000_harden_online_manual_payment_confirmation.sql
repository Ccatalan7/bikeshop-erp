-- Make manual online-order payment confirmation atomic, idempotent, and fully
-- connected to the invoice/payment trace operations.
begin;
...
begin
  if not exists (
    ...
    where conname = 'sales_payments_tenant_id_id_key'
      and conrelid = 'public.sales_payments'::regclass
  ) then
    alter table public.sales_payments
      add constraint sales_payments_tenant_id_id_key unique(tenant_id, id);
  end if;
end $$;

alter table public.online_orders
  add column if not exists manual_payment_id uuid,
  add column if not exists payment_confirmation_operation_id uuid;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'online_orders_manual_payment_fkey'
      and conrelid = 'public.online_orders'::regclass
  ) then
    alter table public.online_orders
      add constraint online_orders_manual_payment_fkey
      foreign key (tenant_id, manual_payment_id)
      references public.sales_payments(tenant_id, id)
      on delete restrict;
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'online_orders_payment_confirmation_operation_fkey'
      and conrelid = 'public.online_orders'::regclass
  ) then
    alter table public.online_orders
      add constraint online_orders_payment_confirmation_operation_fkey
      foreign key (tenant_id, payment_confirmation_operation_id)
      references public.inventory_accounting_operations(tenant_id, id)
      on delete restrict;
  end if;
end $$;

create index if not exists idx_online_orders_manual_payment
  on public.online_orders(tenant_id, manual_payment_id)
  where manual_payment_id is not null;
create index if not exists idx_online_orders_payment_confirmation_operation
  on public.online_orders(tenant_id, payment_confirmation_operation_id)
  where payment_confirmation_operation_id is not null;
...
comment on function public.confirm_online_order_payment(uuid, text, timestamp with time zone) is
  'Atomically confirms one full manual online-order payment under row locks and returns the same payment on replay.';
