-- Fase 8, segundo lector: el inspector del asistente lee el registro.
--
-- `assistant_inspect_inventory_schema_v3` es lo que le dice al modelo qué
-- campos existen, qué valores admiten y cuánta cobertura real hay. Leía
-- `product_spec_values` para la cobertura y `spec_definitions.allowed_values`
-- para el vocabulario — las dos formas viejas.
--
-- Ahora la cobertura sale de `spec_facts` y el vocabulario de
-- `spec_definition_values`. Consecuencia concreta: renombrar una etiqueta llega
-- al asistente en la siguiente consulta, sin reescribir la definición ni los
-- productos.
--
-- El `coalesce` a `allowed_values` se queda a propósito: hay campos cuyo
-- vocabulario todavía no está en el registro, y para ésos el inspector tiene
-- que seguir contestando en vez de quedarse mudo.
--
-- La firma y la proyección no cambian; el gateway desplegado la sigue llamando
-- igual.
