-- Persist the structured supply-request review card under the same closed,
-- server-owned contract used by every durable assistant message. The card is
-- still a review-only projection: it contains no approval or write authority.
...
comment on function public.assistant_cards_valid_v1(jsonb) is
  'Closed durable-card validator, including exact structured supply-request review drafts. Supply drafts carry no approval or write authority.';
