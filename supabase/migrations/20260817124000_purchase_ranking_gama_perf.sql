-- Corrección de rendimiento: la banda de gama se resuelve una vez por tenant.
--
-- La versión 20260817121000 unió `product_gama_v1` directamente dentro de
-- `filtered`. Esa vista deriva las bandas con funciones de ventana sobre todas
-- las métricas de candidatos, así que el planificador la reevaluaba por fila:
-- el ranking del taller real pasó a exceder el `statement_timeout` de 4,5 s y
-- quedó inutilizable. Lo detectó la prueba en la app, no una prueba de esquema:
-- una función puede ser correcta y aun así no servir.
--
-- Acá la banda del tenant se materializa una vez —unas cincuenta marcas— y el
-- join pasa a ser contra esa CTE. El resultado es idéntico; sólo cambia cuántas
-- veces se calcula.
--
