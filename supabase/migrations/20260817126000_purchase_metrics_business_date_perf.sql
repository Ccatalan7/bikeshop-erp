-- El ranking vuelve a caber en su presupuesto: la fecha del negocio se calcula
-- una vez por tenant, no una vez por fila.
--
-- Medición sobre producción, con los 267 candidatos del taller real: el escaneo
-- de `purchase_candidate_metrics_v1` con filtro de texto tardaba **32 s**. El
-- `statement_timeout` del ranking son 4,5 s, así que el camino de texto libre
-- —el que usa el asistente cuando la identidad no está confirmada— estaba
-- inutilizable en producción.
--
-- La causa: `tenant_business_date(aggregate.tenant_id)`. La función es STABLE,
-- pero su argumento es una columna, así que el planificador no puede tratarla
-- como constante y la ejecuta por fila. Peor, dentro del EXISTS de
-- `is_confirmed_local` se llamaba dos veces más por cada fila.
--
-- Este defecto es anterior al trabajo de gama: se descubrió probando el módulo
-- en la app real, no leyendo el esquema. Ninguna prueba de definición lo podía
-- ver, porque la función era correcta — sólo no cabía en su tiempo.
--
-- El resultado es idéntico columna por columna: la fecha del negocio de un
-- tenant es la misma para todas sus filas.
...
with business_day as (
  -- `tenant_business_date` es STABLE pero recibe una **columna**, así que el
  -- planificador no puede constantizarla: se ejecutaba una vez por fila, y dos
  -- veces más dentro del EXISTS de `is_confirmed_local`. Con 267 candidatos del
  -- taller real eso llevaba el escaneo a 32 s, siete veces sobre el
  -- presupuesto del ranking. Acá se evalúa una vez por tenant.
  select tenant.id as tenant_id, public.tenant_business_date(tenant.id) as today
...
select
  ...
  exists (
    ...
    where tag.tenant_id = aggregate.tenant_id
      ...
      and tag.valid_from <= business_day.today
      and (tag.valid_to is null or tag.valid_to >= business_day.today)
  ...
  greatest(business_day.today - latest.latest_purchase_at::date, 0)::integer
...
join business_day
  on business_day.tenant_id = aggregate.tenant_id
join latest
...
comment on view public.purchase_candidate_metrics_v1 is
  'Exact-product historical purchase metrics, latest eligible landed cost, tax-normalized catalog sale basis, product media triple, and explicitly unverified supplier availability. Business date resolved once per tenant.';
