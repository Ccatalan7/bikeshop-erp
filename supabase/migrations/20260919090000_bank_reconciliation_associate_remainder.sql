-- A transfer that paid a registered operation and something more.
--
-- Carlos Sánchez sent $18.000 on 7 July. The ERP has his $7.000 workshop sale
-- FV-00836 of that day, paid «by transfer», and nothing else: the transfer
-- paid that sale and $11.000 nobody registered. The review could only link
-- operations that add up to the movement, so the row stayed pending and the
-- sale appeared as «no aparece en la cartola» (2026-09-19).
--
--   * `associate_existing` accepts an optional `remainder` {account_id,
--     description, reference?}: the operations are linked for their own
--     amounts and what they leave of the movement is one posted journal
--     against the bank to that account (money in: Debe banco / Haber cuenta;
--     money out: Debe cuenta / Haber banco), allocated to the row like any
--     other evidence. Without `remainder` the action is unchanged.
--   * The row still has to add up exactly; a remainder that is zero or
...
comment on function
  ...
  'Applies the open rows of one bank-statement review atomically. Decided rows are final; a movement another statement of the account already settled can only be dismissed as such. A split books one movement as several expenses and one journal; an association may book what its operations leave of the movement as one journal. Generated expenses and journals are posted before their immutable evidence allocation is persisted.';
