-- READ-ONLY classification for retroactive standalone product-form sync noise.
--
-- This splits the previously detected whole-history candidates into:
-- - HIGH CONFIDENCE phantom rows we can auto-clean safely
-- - REVIEW NEEDED rows that may still be legitimate admin stock edits
--
-- High-confidence rules:
-- 1) Non-stock/service products should never get stock adjustments from product form
-- 2) Pure negative-to-zero resyncs are treated as phantom
--
-- Review-needed rules:
-- - Negative-to-positive or negative-to-large-positive batches remain for review
--   because they may be either drift resyncs or real stock corrections.
...
from classified_rows
group by cleanup_class, created_at, created_by
order by created_at desc, cleanup_class;