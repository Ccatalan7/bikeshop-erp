-- A settlement is a settlement because the terminal adapter says so.
--
-- Second pass of Codex's review (2026-09-19). 20260919140000 told card
-- settlements apart from manual links by their provider, and that is not
-- what makes one: a `manual` allocation with provider `transbank` sent
-- straight to the RPC reconciles a $50.000 sale with a $48.750 deposit
-- while the adapter, which only books a settlement when it receives
-- `processor_estimate`, posts nothing — the commission and the transfer out
-- of the clearing account never happen. It also left the legacy paid
-- expense open: its own journal (`source_module = 'expenses'`) could be
-- linked apart from the `expense` target, explaining one payment twice.
--
--   * The adapter marks every estimate it turns into a manual allocation
--     with `settlement: true`, and strips that key from anything else, so
--     only the settlement path can claim it.
--   * The kernel bounds every other manual allocation at $1.000, whatever
--     its provider, and lets a marked settlement be below its sale, never
--     above.
--   * A journal posted by the expenses module joins the payment journals
--     the kernel refuses as targets of their own.
--
    ...
    if v_action_code = 'associate_existing' then
      ...
      loop
        if jsonb_typeof(v_allocation) <> 'object'
           ...
           or (
             (
               ...
               or (
                 ...
                 and coalesce(v_allocation->>'settlement', 'false') <> 'true'
               )
             )
             and abs(
           ...
           -- A card settlement (the terminal adapter marks the estimates
           -- it turns into manual allocations, and books the acquirer's
           -- commission for them) nets that commission: never more than
           -- the sale.
           or (
             (
               v_allocation->>'match_kind' = 'transbank_estimate'
               or coalesce(v_allocation->>'settlement', 'false') = 'true'
             )
          ...
          select 1 from public.journal_entries entry
           where entry.tenant_id = v_tenant_id
             ...
             and coalesce(entry.source_module, '') in (
               ...
               'payment_terminal_settlements', 'expenses'
             )
        ) then
