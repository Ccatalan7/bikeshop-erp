-- Contactar a un cliente por WhatsApp tiene una regla que el operador no
-- debería tener que recordar: fuera de las 24 horas desde su último mensaje
-- entrante, Meta sólo acepta plantillas aprobadas. Hoy eso se descubre al
-- intentar escribir.
--
-- Esta función resuelve el hecho —quién es el cliente, qué conversación tiene y
-- si la ventana está abierta— para que el asistente ofrezca el camino correcto
-- antes de escribir. NO envía nada: el envío sigue ocurriendo por el camino
-- autorizado del cliente, después de que el operador confirme el previsualizado.
...
revoke all on function public.assistant_prepare_customer_contact_v1(text, integer) from public;
grant execute on function public.assistant_prepare_customer_contact_v1(text, integer) to authenticated;
