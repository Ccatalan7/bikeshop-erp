-- Revert stock_movements_view to query Source Tables directly (since stock_movements ledger is empty/incomplete)
-- improvements:
-- 1. Include Mechanic Jobs (uninvoiced) to capture stock used in workshop.
-- 2. Filter Sales Invoices for 'confirmed'/'paid' only.
-- 3. Calculate running balance dynamically from current stock to ensure continuity.
...
with all_movements as (
  ...
  -- 3. Mechanic Jobs (OUT) - UNINVOICED ONLY
  -- (If invoiced, the Sales Invoice above covers it)
  select 
    mji.id,
    mji.product_id,
    coalesce(mji.product_name, p.name) as product_name,
    coalesce(mji.product_sku, p.sku) as product_sku,
    mj.created_at as transaction_date, -- Use job creation or updated date?
    ...
    mj.notes,
    null::uuid as created_by, -- mj doesn't have created_by standardly exposed?
    mji.created_at,
    mj.tenant_id
  ...
  where 
    mj.invoice_id is null -- Avoid double counting if invoiced
    and mj.status not in ('borrador', 'draft', 'cancelado', 'cancelled') -- Active jobs only
    and (mji.item_type = 'product' or mji.item_type is null) -- Physical items only
  ...
  select 
    ...
    sa.quantity, -- Already signed? Check table check constraint. Usually manual is signed.
    sa.reason as notes,
    sa.created_by,
    sa.created_at,
    sa.tenant_id
