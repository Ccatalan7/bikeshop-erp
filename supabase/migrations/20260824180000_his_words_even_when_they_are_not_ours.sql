-- El último escalón: sus palabras, aunque no sean las nuestras.
--
-- Quedaban dos ceros que no eran falta de datos, sino de vocabulario nuestro:
--
--   «aros 26»         → «aro» no es ninguna categoría del catálogo; eso se
--                        llama «Llantas». La palabra del operador no aparece
--                        en ninguna rama, así que ningún escalón anterior
--                        tenía de dónde agarrarse.
--   «platos y bielas» → son dos cosas distintas, y exigir las dos palabras en
--                        UN mismo nombre de producto no calza con ninguno.
--
-- El escalón final pide UNA palabra suya de cuatro letras o más, no todas.
-- Sigue mandando su vocabulario: «qué me falta comprar» no calza con ningún
-- producto y se queda en cero, que ahí sí es la respuesta honesta.

begin;

-- La raíz de una palabra del taller, en un solo lugar.
--
-- «plato» devolvía historial y «platos» cero; «bielas» devolvía historial y
-- «biela» cero. Depende de en qué número quedó escrito ese producto, que es lo
-- único que el operador no puede saber.
create or replace function public.purchase_word_stem_internal_v1(p_token text)
returns text
language sql
immutable
parallel safe
...
as $$
  select case
    when length(p_token) > 4 and p_token like '%es'
      then left(p_token, length(p_token) - 2)
    when length(p_token) > 3 and p_token like '%s'
      then left(p_token, length(p_token) - 1)
    else p_token
  end
$$;

revoke all on function public.purchase_word_stem_internal_v1(text)
...
begin
  ...
  for v_attempt in 1..4 loop
    ...
    if v_attempt = 2 then
      ...
      -- **El último escalón: sus palabras, aunque no sean las nuestras.**
      --
      -- «aros 26» y «platos y bielas» llegaban aquí en cero, y no por falta de
      -- datos: «aro» no es ninguna categoría nuestra —eso se llama «Llantas»—
      -- y «platos y bielas» son dos cosas distintas, así que exigir las dos
      -- palabras en UN mismo nombre no calza con ninguna.
      --
      -- Se pide UNA palabra suya de al menos cuatro letras, no todas. Sigue
      -- siendo su vocabulario el que manda: «qué me falta comprar» no calza
      -- con ningún producto y se queda en cero, que es la respuesta honesta.
    ...
    with recursive category_scope as (
      ...
      where predicate_state.predicates_match
        ...
        and (
          ...
          --
          -- En el último escalón basta UNA palabra suya de cuatro letras o
          -- más, en vez de todas: «platos y bielas» nombra dos cosas y ningún
          -- producto se llama las dos.
          or (
            case when v_match_any then
              exists (
                ...
                where length(token) >= 4
                  and (
                    position(token in purchased.identity_surface) > 0
                    or position(
                      public.purchase_word_stem_internal_v1(token)
                      in purchased.identity_surface
                    ) > 0
                  )
              )
            else
