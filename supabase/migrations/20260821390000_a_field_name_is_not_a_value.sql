-- «motor caja 73» amarraba BSA sin que nadie lo pidiera: «caja» aparece dentro
-- de un solo valor del vocabulario, pero es ante todo la palabra del rótulo del
-- campo —«Caja de motor»—. Una palabra que nombra un campo deja de ser
-- evidencia de un valor.
--
-- Y cuando una palabra sí es de un valor pero vive en varios campos —«bsa» está
-- en cuatro—, decide el catálogo: gana el campo que el taller realmente llena,
-- y sólo si gana solo.
