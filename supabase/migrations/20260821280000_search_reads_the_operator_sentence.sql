...
-- caja 68 y largo de eje 118». Hasta ahora esa frase entraba al buscador como
-- texto libre, y el filtro de texto exige que *cada* palabra esté en el nombre
-- del producto — «caja» y «bsa» no lo están, así que la respuesta era cero.
--
-- Con el vocabulario convertido en filas (`spec_definition_values`) el servidor
-- ya puede traducir esa frase él mismo: reconoce las palabras que son valores
-- del registro, amarra los números a su campo, y descarta el resto. No se le
-- enseña caso por caso ni se depende de que el modelo arme el predicado.
--
-- Reglas, de la más fuerte a la más débil:
--   1. Una palabra que aparece en un solo valor de todo el vocabulario nombra
--      ese valor.
--   2. Un número precedido —dentro de cuatro palabras— por una palabra que
--      aparece en un solo rótulo del alcance, se amarra a ese campo.
--   3. Un número suelto se amarra si dentro del alcance hay un solo campo cuyos
--      hechos reales lo contengan.
-- El alcance es el más angosto que la frase justifique: la plantilla que probó
-- el vocabulario, o —si no hubo— las plantillas de la categoría nombrada.
-- Unir ambos ensancharía el alcance y volvería ambiguo un rótulo genérico como
-- «Ancho», que existe en llantas, neumáticos y cajas por igual.
...
as $function$
  with params as (
    ...
    -- La misma superficie que usa el filtro de texto del buscador: si se mide
    -- contra otra, un residuo «válido» aquí deja fuera al producto allá.
    ...
    from public.products_with_sets pr
    cross join params p
    where pr.tenant_id = p.tid
      and pr.is_active is true
      and (
        not exists (select 1 from query_categories)
        or pr.category_id in (select id from query_categories)
      )
  ), residual_tokens as (
    ...
    where t.ordinality not in (select ordinality from consumed)
      and t.token ~ '^[a-z]{4,}$'
      and exists (
        select 1 from scoped_surfaces s
        where s.surface ~ ('(^| )' || t.token || '( |$)')
      )
  )
...
revoke all on function public.assistant_infer_technical_predicates_internal_v1(
  uuid, text
) from public;

comment on function public.assistant_infer_technical_predicates_internal_v1(
  uuid, text
) is
  'Traduce la frase del operador a predicados técnicos usando el registro de '
  'vocabulario y los hechos reales. Devuelve {predicates, residual}.';
