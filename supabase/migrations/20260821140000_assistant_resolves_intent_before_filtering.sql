-- El asistente interpreta primero y filtra después.
--
-- Hasta acá el buscador exigía que el valor pedido fuera IDÉNTICO a la entrada
-- del vocabulario, y si no lo era, `raise exception 'Invalid AI tool
-- arguments'` botaba la consulta completa. Con un vocabulario compuesto como
-- `BSA / Caja inglesa 34,8 mm (1.37") x 24`, eso significa que sólo funciona si
-- quien pregunta reproduce el string entero. Un mecánico dice «caja inglesa».
--
-- Reproducido tres veces seguidas: «un motor para caja inglesa de 68 mm con eje
-- cuadrado de 118 mm» fallaba, mientras «ancho de caja 68 y largo de eje 118»
-- funcionaba. La diferencia era la forma de nombrar, no el dato.
--
-- Dos cambios, y los dos son de arquitectura, no de este caso:
--
--   1. TRADUCIR. El valor que llega se resuelve contra `allowed_values` antes
--      de filtrar: coincidencia exacta, y si no, la única entrada que lo
--      contiene o que está contenida en él. «caja inglesa» encuentra la caja
--      inglesa; «cuadrado» encuentra el cuadrado. Eso es trabajo del buscador,
--      no del operador ni del modelo.
--
--   2. DESCARTAR, NO ABORTAR. Un predicado que no se puede traducir se cae
--      solo y la consulta sigue con los que sí sirven. Pedir tres cosas y
--      equivocarse en una tiene que devolver las otras dos, no «no
--      disponible» para todo.
--
-- Lo que NO cambia: un valor ambiguo —que calce con dos entradas distintas— no
-- se resuelve. Adivinar entre dos estándares es peor que no filtrar.
--
-- La firma y el sobre de resultado quedan iguales, así que el gateway
-- desplegado la sigue llamando sin cambios.

begin;

create or replace function public.assistant_resolve_select_value_internal_v1(
  p_allowed jsonb,
  p_requested text
) returns jsonb
language sql
...
as $resolve$
  with pedido as (
    select public.assistant_normalize_query_internal_v1(p_requested) as texto
  ),
  vocabulario as (
    select entrada.value,
      public.assistant_normalize_query_internal_v1(entrada.value #>> '{}') as texto
    from jsonb_array_elements(coalesce(p_allowed, '[]'::jsonb)) entrada(value)
  ),
  exacta as (
    select v.value from vocabulario v, pedido p where v.texto = p.texto limit 1
  ),
  contenida as (
    -- La entrada del vocabulario contiene lo pedido: «caja inglesa» dentro de
    -- «BSA / Caja inglesa 34,8 mm (1.37") x 24».
    select array_agg(v.value) as candidatas
    from vocabulario v, pedido p
    where p.texto <> '' and position(p.texto in v.texto) > 0
  ),
  contiene as (
    -- Lo pedido contiene la entrada: «cuadrado JIS de 118» sobre «Cuadrado JIS».
    select array_agg(v.value) as candidatas
    from vocabulario v, pedido p
    where v.texto <> '' and position(v.texto in p.texto) > 0
  )
  select case
    when (select value from exacta) is not null then (select value from exacta)
    -- Una sola candidata es una traducción; dos son una adivinanza y no se hace.
    when array_length((select candidatas from contenida), 1) = 1
      then ((select candidatas from contenida))[1]
    when array_length((select candidatas from contiene), 1) = 1
      then ((select candidatas from contiene))[1]
    else null
  end;
$resolve$;

comment on function public.assistant_resolve_select_value_internal_v1(jsonb, text) is
  'Traduce el valor que trae el asistente al término real del vocabulario. '
  'Devuelve null cuando no hay traducción o cuando hay más de una candidata: '
  'adivinar entre dos estándares es peor que no filtrar.';
...
begin
  ...
  loop
    ...
    if v_definition.data_type in ('single_select', 'multi_select')
      ...
      select
        jsonb_agg(traducido order by orden) filter (where traducido is not null),
        count(*) filter (where traducido is null)
      into v_translated_values, v_untranslated
      from (
        select pedido.ordinality as orden,
          public.assistant_resolve_select_value_internal_v1(
            v_definition.allowed_values, pedido.value #>> '{}'
          ) as traducido
        from jsonb_array_elements(v_values) with ordinality
          as pedido(value, ordinality)
      ) resolucion;

      -- Lo que no se puede traducir se descarta; no bota la consulta entera.
      -- Pedir tres cosas y equivocarse en una tiene que devolver las otras dos.
      if v_untranslated > 0 or v_translated_values is null then
