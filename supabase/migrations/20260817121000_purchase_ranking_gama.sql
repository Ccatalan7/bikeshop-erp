-- La gama entra al ranking: ordena, nunca elimina.
--
-- El kernel devolvia la marca pero no la puntuaba: ninguna de sus seis
-- dimensiones era marca ni rango de precio, asi que «gama economica» dependia
-- de que el modelo interpretara una frase. Con `p_gama` pasa a ser una
-- dimension del motor, resuelta contra la banda vigente de `product_gama_v1`
-- —derivada del historial de compras y corregible por el dueño—.
--
-- Sin `p_gama` el puntaje es identico al anterior: el peso solo se redistribuye
-- cuando alguien pidio una gama.
--
-- Esta definicion parte de `20260816170000_purchase_candidate_media_contract`,
-- que es la vigente. Generarla desde el kernel original habria revertido el
-- contrato de imagenes del candidato; lo detecto su propia prueba pgTAP.

begin;

-- El parametro nuevo cambia la firma. Se retira la anterior para que PostgREST
-- no quede con dos candidatas ambiguas ante la misma llamada de cinco claves.
drop function if exists public.rank_purchase_candidates_v1(
...
begin
  ...
  with recursive category_scope as (
    ...
    left join public.product_gama_v1 gama
      on gama.tenant_id = metric.tenant_id
     and gama.category_id = metric.category_id
     and lower(btrim(gama.brand)) = lower(btrim(metric.brand))
...
comment on function public.rank_purchase_candidates_v1(
  ...
  'Tenant-bound, explainable historical purchase candidate ranking. Balances landed margin, observed history, recency, stability, evidence and, when requested, gama band. Supplier availability is always unverified.';
