-- Fix payroll expense accounting so accrual entries credit 2106 and payment JEs
-- continue to settle that liability using the payment row id as source_reference.

-- 1) Repair historical payroll expense journal entries that credited cash/bank
-- instead of 2106 Sueldos por Pagar.
...
join public.expenses e
  on e.tenant_id = je.tenant_id
 and e.expense_number = je.source_reference
...
where jl.entry_id = je.id
  ...
  and jl.account_code in ('1101', '1110', '2105');

-- 2) Keep source of truth in sync with the live payroll behavior.
