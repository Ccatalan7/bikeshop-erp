-- Se suelta la medida que no tiene cobertura, no todas.
--
-- El panel «Por qué» destapó esto en la app real, que es exactamente para lo
-- que existe: «Cámaras 29 Schrader» mostraba como evidencia cámaras de 26 y de
-- 700c. La frase pedía `wheel_size = 29` —que SÍ calza, seis productos— y
-- `valve_type = Schrader`, que en esa rama no está poblado. El escalón que
-- suelta la medida las soltaba EN BLOQUE, así que perdía el 29 junto con el
-- Schrader y pasaba de 6 productos a 29.
--
-- Una medida que calza es lo que distingue la petición; una que no calza es
-- sólo un campo sin poblar. Se conserva la primera y se suelta la segunda.
--
-- Si todas tenían cobertura, lo que falla es su COMBINACIÓN y este escalón ya
-- no aplica: se pasa al siguiente en vez de ensanchar sin motivo.
...
begin
  ...
  for v_attempt in 1..4 loop
    ...
    if v_attempt = 2 then
      ...
      -- **Se suelta la medida que no tiene cobertura, no todas.**
      --
      -- Antes este escalón vaciaba los predicados en bloque. Medido en la app
      -- real: «Cámaras 29 Schrader» pedía `wheel_size=29` —que SÍ calza, seis
      -- productos— y `valve_type=Schrader`, que en esa rama no está poblado.
      -- Soltar las dos dejaba 29 productos: cámaras de 26 y de 700c
      -- presentadas como evidencia de una compra de 29. El operador lo vio al
      -- abrir el detalle, que es justo para lo que sirve.
      --
      -- Una medida que calza es lo que distingue la petición; una que no
      -- calza es sólo un campo sin poblar. Se conserva la primera.
      select coalesce(jsonb_agg(predicate.value), '[]'::jsonb)
      into v_predicates
      ...
      where exists (
        ...
        where product.tenant_id = p_tenant_id
          ...
          and public.assistant_inventory_technical_predicate_source_internal_v1(
            ...
            coalesce(predicate.value -> 'values', '[]'::jsonb),
      ...
      -- Si todas tenían cobertura, la contradicción está en su combinación y
      -- este escalón no aplica: se pasa al siguiente en vez de ensanchar.
      continue when jsonb_array_length(v_predicates) > 0
        and v_predicates = coalesce(v_inferred -> 'predicates', '[]'::jsonb);
      v_dropped_filters := 'la medida sin cobertura';
