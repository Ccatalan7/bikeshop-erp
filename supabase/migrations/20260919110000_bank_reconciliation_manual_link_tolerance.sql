-- An operation chosen by hand is linked for what it is.
--
-- «Vincular operación» lets the operator pick several ERP operations for one
-- movement; the client gives each its own amount and the last what is left.
-- The kernel only bounded that difference for direct matches, so an
-- association that did not add up (Carlos Sánchez's $18.000 transfer against
-- his $7.000 sale) could reach it as $18.000 of bank evidence for a $7.000
-- sale and be accepted: the other $11.000 would never be booked. None was
-- applied (read back 2026-09-19: every manual allocation equals its target).
--
--   * A manual allocation, like a direct one, may differ from its operation by
--     at most $1.000 (a transfer fee or a rounding). What an association
--     leaves beyond that is booked with `remainder` (20260919090000).
--
    ...
    if v_action_code = 'associate_existing' then
      ...
      loop
        if jsonb_typeof(v_allocation) <> 'object'
           ...
           or (
             v_allocation->>'match_kind' in ('direct', 'manual')
             and abs(
