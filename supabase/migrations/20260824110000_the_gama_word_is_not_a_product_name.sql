-- «Gama media» no es el nombre de ningún producto.
--
-- La primera versión dejaba las palabras de gama en el texto libre, y el
-- filtro exige que CADA palabra esté en el nombre del producto. Resultado
-- medido: «neumáticos 29 de gama media y alta» —el ejemplo textual del
-- dueño— devolvía CERO proveedores teniendo seis, mientras «cámaras 29»
-- funcionaba. La palabra con la que él habla mataba la consulta.
--
-- Ahora la banda se consume como señal: informa (`gamaMix`,
-- `requestedGamaLines`) y nunca filtra. Es la misma regla que gobierna el
-- resto del módulo — la gama ordena, jamás elimina.
...
--    en `Componentes / Ruedas / Rayos` es Derman con 57% sobre 4. Por eso el
--    alcance se resuelve con el mismo traductor de frases que ya usa
--    `search_inventory`, y no subiendo a la categoría padre.
--
...
--
-- La gama NO elimina: se informa como mezcla por proveedor. Preguntar por
-- «gama media y alta» no puede borrar al proveedor cuya banda no se pudo
-- derivar, porque no saber no es no calzar.
--
...
begin
  ...
  v_scope := jsonb_build_object(
    ...
    'requestedGama', coalesce(to_jsonb(v_requested_gamas), '[]'::jsonb)
  );
