-- `inspect_inventory_schema` devolvía 43 ítems y el ejecutor corta en 40.
--
-- La consulta del catálogo se limitaba a 40 y DESPUÉS se agregaban tres campos
-- operativos fijos (stock, stock mínimo, precio). Con cualquier consulta amplia
-- —«freno», «rueda», «piñón» dan 43— el sobre se pasaba del tope y el ejecutor
-- lo rechazaba entero: la herramienta fallaba precisamente en las preguntas más
-- comunes, y es la que resuelve la categoría antes de buscar.
--
-- Se reparte el tope: 37 del catálogo más los 3 fijos. `hasMore` ya existía y
-- sigue avisando que la lista viene cortada.
