-- A classification of money going out booked the bank backwards.
--
-- `post_journal` writes two lines: the debited side (the bank for money in,
-- the counterpart account for money out) and the credited side. The amounts
-- were chosen by direction a second time, so for money out the first line
-- (the account) got the credit and the second (the bank) the debit: every
-- bank fee, owner withdrawal or other classified charge raised the bank
-- instead of lowering it. Money in was right, and the only test covered money
-- in. It surfaced on the first real apply (2026-09-19): 13 charges ($31.443:
-- the owner's subscriptions to 3103 and a bank fee to 6601) left the bank
-- ledger $62.886 above the statement. Splits, expenses and the association
-- remainder were already right.
--
--   * The first line is always the debit and the second the credit.
--   * The journals it already wrote are corrected in place, swapping debit and
--     credit on each line: they are the evidence the statement rows point to,
--     so their allocation must keep pointing at a right journal. Only journals
--     of a `post_journal` decision on money out whose bank line is a debit are
--     touched, so running this again changes nothing.
--
...
begin
  with wrong as (
    select distinct decision.generated_target_id as entry_id
      from public.bank_reconciliation_row_decisions decision
     ...
     where decision.action_kind = 'post_journal'
       ...
       and statement_row.direction = 'debit'
       and bank_line.debit_amount > 0
  )
  update public.journal_lines line
     set debit_amount = line.credit_amount,
         credit_amount = line.debit_amount
    from wrong
   where line.entry_id = wrong.entry_id;
  get diagnostics v_fixed = row_count;
  raise notice 'líneas corregidas: %', v_fixed;
end;
$$;
