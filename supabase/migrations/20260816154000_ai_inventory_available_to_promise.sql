...
-- xzdvtzdqjeyqxnkqprtf on 2026-08-16; live ATP parity read-back passed.
-- Reservation-aware inventory search for the general AI agent.
-- Forward: add v7 with the same closed query language as v6, but make every
-- stock predicate, order, summary and returned quantity use common ATP.
-- Recovery: point the gateway back to assistant_search_inventory_v6.
-- Risk: additive read-only function; no stock or reservation mutation.
begin;

create or replace function public.assistant_search_inventory_v7(
  p_query text,
  p_category text,
  p_availability text,
...
revoke all on function public.assistant_search_inventory_v7(
  text, text, text, jsonb, jsonb, text, text, integer, text
) from public, anon, authenticated, service_role;
grant execute on function public.assistant_search_inventory_v7(
  text, text, text, jsonb, jsonb, text, text, integer, text
) to authenticated;

comment on function public.assistant_search_inventory_v7(
  text, text, text, jsonb, jsonb, text, text, integer, text
) is
  'Tenant-bound inventory query whose availability filters, stock ordering, totals and returned stock all use common available-to-promise after online and workshop commitments.';
