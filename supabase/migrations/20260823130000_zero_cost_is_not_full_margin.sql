-- Costo cero no es margen del 100%.
--
-- «¿Qué producto me deja más margen?» es una pregunta corriente del taller y el
-- asistente respondía que no tenía herramienta para contestarla — teniendo el
-- dato en la misma fila que ya leía. El buscador proyectaba `price` y no
-- `cost`, así que el margen no era calculable aguas arriba.
--
-- Medido antes de agregarlo: 1.520 de 1.612 productos activos tienen precio y
-- costo, así que la respuesta es real y no un hueco lleno de nulos.
--
-- `marginPercent` es sobre el precio de venta, que es como lo mira el taller, y
-- queda NULO cuando falta un dato: un margen inventado es peor que ninguno.
--
-- Costo cero NO es margen 100%. De los 91 productos activos con costo cero, 58
-- son servicios —mano de obra, sin costo de compra— y 33 son productos con el
-- costo sin registrar. Contarlos como 100% hacía que «cuál me deja más margen»
-- devolviera cinco servicios empatados en 100% y ningún producto real: exacto
-- en la aritmética e inútil como respuesta. Sin costo, no hay margen que
-- comparar.
-- `sort.field = 'margin'` permite pedir directamente los de mayor o menor
-- margen sin que el modelo reordene texto.
...
begin
  ...
  if octet_length(coalesce(p_query, '')) > 240
     ...
     or p_sort_field not in ('relevance','name','stock','minimum_stock','price','margin')
        ...
        from (
          select definition.label,
            ...
            coalesce(
              ...
              case when fact.value_boolean then 'Sí' else 'No' end
            ) value
