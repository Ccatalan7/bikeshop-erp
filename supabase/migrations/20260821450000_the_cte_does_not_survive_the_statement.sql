-- `assistant_rank_sales_customers_v1` reventaba en cada llamada: el `return`
-- referenciaba el CTE `ranked`, que sólo existe dentro del query anterior. El
-- gateway lo devolvía como `assistant_unavailable:500`, sin decir qué falló.
--
-- Se corrige resolviendo el conteo dentro del mismo query. La lección real es
-- otra: la RPC se cableó como herramienta sin haberla ejecutado ni una vez.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    -- El conteo se resuelve DENTRO de este query: `ranked` es un CTE y no
    -- existe en la sentencia siguiente. Referenciarlo en el `return` hacía
    -- reventar la función entera con «relation ranked does not exist», y el
    -- gateway lo devolvía como un 500 opaco.
    count(*)::integer
  into v_items, v_total, v_rows
  from ranked;

  return public.assistant_tool_envelope_internal_v1(
    v_authority.tenant_id, v_items, coalesce(v_rows, 0) > p_limit
  );
end;
$function$;
