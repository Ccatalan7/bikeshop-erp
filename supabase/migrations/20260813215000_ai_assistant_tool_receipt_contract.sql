-- Centralize the durable receipt contract so each model-visible primitive has
-- one explicit risk, policy and result-count ceiling. Schema discovery returns
-- up to 40 rows; ordinary business reads remain capped at 10.
begin;
...
begin
  ...
  from assistant_runtime.assistant_tool_receipt_contract_internal_v1(p_tool_name);
  ...
  if p_ordinal <> coalesce((
     ...
     or v_contract.max_result_count is null
     or p_result_count not between 0 and v_contract.max_result_count
     ...
     or p_risk <> v_contract.risk
     or p_policy_decision <> v_contract.policy_decision
...
comment on function assistant_runtime.assistant_record_tool_receipt_v1(
  ...
  text, text, text, text, text, integer, integer, boolean, boolean, text,
  timestamptz, timestamptz
) is 'Internal durable receipt writer bound to the typed assistant tool contract.';
