-- Un término ambiguo amplía la búsqueda; no la cancela.
--
-- La versión anterior devolvía null cuando el término del operador calzaba con
-- más de una entrada del vocabulario, y ese predicado se descartaba. La idea
-- era «no adivinar entre dos estándares», pero está mal planteada: si el
-- operador pide «cuadrado» y existen «Cuadrado JIS» y «Cuadrado ISO», los DOS
-- son cuadrados. Devolver los dos no es adivinar — es contestar lo que
-- preguntó. Rendirse ahí le esconde stock que sí le sirve.
--
-- Decisión del dueño, 2026-08-21: «si no le especifico eso, por qué mierda se
-- rinde y no simplemente me arroja todos los resultados, incluidos JIS e ISO».
--
-- Ahora el traductor devuelve TODAS las candidatas y el buscador compara por
-- pertenencia. El predicado sólo se descarta cuando ningún término se pudo
-- traducir a nada, que es el único caso en que de verdad no hay qué filtrar.
...
as $resolve$
  ...
  exacta as (
    select v.value from vocabulario v, pedido p where v.texto = p.texto
  ),
  -- La entrada contiene lo pedido: «cuadrado» dentro de «Cuadrado JIS» y de
  -- «Cuadrado ISO». Las dos son respuesta.
  contenida as (
    select v.value from vocabulario v, pedido p
  ...
  elegido as (
    select value from exacta
    union all
    select value from contenida where not exists (select 1 from exacta)
    union all
    select value from contiene
      where not exists (select 1 from exacta)
        and not exists (select 1 from contenida)
  )
  select coalesce(jsonb_agg(distinct value), '[]'::jsonb) from elegido;
$resolve$;

comment on function public.assistant_resolve_select_value_internal_v1(jsonb, text) is
  'Traduce el término del operador a TODAS las entradas del vocabulario que lo '
  'satisfacen. «cuadrado» devuelve JIS e ISO: las dos son cuadrado, y filtrar '
  'por ambas es contestar la pregunta, no adivinarla. Devuelve [] sólo cuando '
