-- El asistente no podía filtrar hablando como mecánico.
--
-- `assistant_search_inventory_v7` exige que un valor pedido sobre un campo de
-- lista coincida con la entrada COMPLETA de `allowed_values`, y si no,
-- `raise exception 'Invalid AI tool arguments'` bota la llamada entera. El
-- asistente entonces reporta la fuente como no disponible — correctamente, sin
-- inventar, pero sin contestar.
--
-- Reproducido dos veces seguidas: «un motor para caja inglesa de 68 mm con eje
-- cuadrado de 118 mm» falla, mientras «Construcción sea Rodamiento sellado» y
-- «ancho de caja 68 y largo de eje 118» funcionan. La diferencia es que las que
-- funcionan nombran el valor completo y exacto.
--
-- La causa es el vocabulario que desplegué hoy: los valores son compuestos
-- (`BSA / Caja inglesa 34,8 mm (1.37") x 24`), y exigir el string entero deja
-- fuera cualquier forma en que un mecánico nombre la pieza.
--
-- El comparador ya sabe hacer `contains` sobre campos de lista — está en
-- `assistant_inventory_technical_predicate_source_internal_v1`, en la misma
-- rama que atiende `single_select` y `multi_select`. Era sólo el validador el
-- que no lo dejaba pasar.
--
-- Este cambio hace exactamente dos cosas sobre la definición vigente:
--   1. admite `contains` como operador para campos de lista;
--   2. salta la comprobación de pertenencia a `allowed_values` cuando el
--      operador es `contains`, porque un fragmento nunca va a ser una entrada.
--
-- `eq`, `neq` e `in` conservan la comprobación exacta: un filtro que afirma un
-- valor concreto sigue teniendo que nombrar uno que existe.
       ...
       or (v_definition.data_type in ('single_select','multi_select') and (
         ...
         or exists (
           ...
           where jsonb_typeof(requested.value) <> 'string'
             or (
               v_operator <> 'contains'
               and jsonb_array_length(v_definition.allowed_values) > 0
               and not exists (
                 select 1
                 from jsonb_array_elements(
                   v_definition.allowed_values
                 ) allowed(value)
