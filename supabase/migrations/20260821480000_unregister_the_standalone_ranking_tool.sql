-- `rank_sales_customers` se retiró de la superficie del modelo: su desglose
-- vive ahora dentro de `analyze_sales_period`, que el modelo sí usa. El
-- allowlist de recibos deja de anunciarla para que no quede una herramienta
-- registrada que ninguna capa ofrece.
--
-- La RPC `assistant_rank_sales_customers_v1` se conserva: es correcta, está
-- verificada y sirve para cablearla a una pantalla del ERP sin depender del
-- criterio del modelo.
