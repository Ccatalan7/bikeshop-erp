...
begin
  -- Check if mechanic_jobs table exists
  select exists (
  ...
  if not v_table_exists then
    raise notice 'Skipping mechanic_jobs migration - table does not exist yet';
    return;
  end if;
...
exception
  when others then
    raise notice 'Mechanic jobs migration failed (table may not exist): %', sqlerrm;
end $$;
...
--------------------------------------------------------------------------------
-- Utility trigger function: keep updated_at in sync for any table
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Utility trigger function: auto-populate created_by with auth.uid()
--------------------------------------------------------------------------------
...
do $$
declare
  v_purchase_payments_exists boolean := to_regclass('public.purchase_payments') is not null;
begin
  -- Drop all triggers
  if v_purchase_payments_exists then
...
-- ==========================================================================
-- PRODUCTS CORE TABLE (must exist before any FK references)
-- ==========================================================================
...
-- Migration: Add missing columns to products table (idempotent)
do $$
begin
  ...
  -- Legacy brand/model text fields
...
-- ============================================================================
-- SUPPLIERS TABLE - Contacts / Vendors
-- ============================================================================
...
-- Ensure supplier-dependent columns/constraints on products are created after suppliers table exists
do $$
begin
  if not exists (
    select 1
...
create table if not exists smart_purchase_list (
  ...
  linked_expense_id uuid, -- FK added later after expenses table definition
...
begin
  ...
  -- Drop new version with composite type parameter only if the table exists
  if v_purchase_payments_exists then
    execute 'drop function if exists public.create_purchase_payment_journal_entry(public.purchase_payments) cascade';
    execute 'drop function if exists public.create_purchase_payment_journal_entry(p_payment public.purchase_payments) cascade';
  end if;
end $$;
...
-- Ensure smart_purchase_list.linked_expense_id enforces FK after expenses table exists
do $$
begin
  if exists (
    select 1
     ...
     where table_schema = 'public'
       ...
       and column_name = 'linked_expense_id'
  ) then
    if not exists (
      select 1
        from pg_constraint
...
exception
  when undefined_table then
    raise notice 'smart_purchase_list table not found when adding FK to expenses';
end $$;
...
-- handle_expense_line_change() keeps expense journal entries and totals in sync
...
-- Ensure wheel_builds references bikes only after bikes table exists
do $$
begin
  if not exists (
    select 1
      from information_schema.columns
     where table_schema = 'public'
       and table_name = 'wheel_builds'
       and column_name = 'bike_id'
  ) then
    alter table wheel_builds add column bike_id uuid;
  end if;

  if not exists (
    select 1
      from pg_constraint
     where conrelid = 'public.wheel_builds'::regclass
       and conname = 'wheel_builds_bike_id_fkey'
  ) then
    alter table wheel_builds
      add constraint wheel_builds_bike_id_fkey
...
-- Ensure wheel_builds references mechanic_jobs once table exists
do $$
begin
  if not exists (
    select 1
      from pg_constraint
     where conrelid = 'public.wheel_builds'::regclass
       and conname = 'wheel_builds_mechanic_job_id_fkey'
  ) then
    alter table wheel_builds
      add constraint wheel_builds_mechanic_job_id_fkey
        foreign key (mechanic_job_id) references public.mechanic_jobs(id) on delete set null;
  end if;
end $$;
...
--------------------------------------------------------------------------------
-- User invitations table (depends on employees)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- BACKUP & RESTORE FUNCTIONS (TEMPORARILY DISABLED FOR FRESH DEPLOYMENTS)
--------------------------------------------------------------------------------
-- Note: These functions are commented out because they reference tables that may
-- not exist yet on fresh database deployments (employees, mechanic_jobs, bikes, etc.).
-- PostgreSQL validates function bodies at creation time, causing deployment failures.
-- Uncomment and deploy separately after all core tables are created.
...
begin
  -- Collect summary statistics (with error handling for tables that may not exist yet)
  begin select count(*) into v_product_count from products where tenant_id = p_tenant_id; exception when undefined_table then v_product_count := 0; end;
  begin select count(*) into v_product_category_count from product_categories where tenant_id = p_tenant_id; exception when undefined_table then v_product_category_count := 0; end;
  begin select count(*) into v_customer_count from customers where tenant_id = p_tenant_id; exception when undefined_table then v_customer_count := 0; end;
  begin select count(*) into v_supplier_count from suppliers where tenant_id = p_tenant_id; exception when undefined_table then v_supplier_count := 0; end;
  begin select count(*) into v_sales_invoice_count from sales_invoices where tenant_id = p_tenant_id; exception when undefined_table then v_sales_invoice_count := 0; end;
  begin select count(*) into v_purchase_invoice_count from purchase_invoices where tenant_id = p_tenant_id; exception when undefined_table then v_purchase_invoice_count := 0; end;
  begin select count(*) into v_employee_count from employees where tenant_id = p_tenant_id; exception when undefined_table then v_employee_count := 0; end;
  begin select count(*) into v_journal_entry_count from journal_entries where tenant_id = p_tenant_id; exception when undefined_table then v_journal_entry_count := 0; end;
  begin select count(*) into v_mechanic_job_count from mechanic_jobs where tenant_id = p_tenant_id; exception when undefined_table then v_mechanic_job_count := 0; end;
  begin select count(*) into v_bike_count from bikes where tenant_id = p_tenant_id; exception when undefined_table then v_bike_count := 0; end;
  begin select count(*) into v_product_brand_count from product_brands where tenant_id = p_tenant_id; exception when undefined_table then v_product_brand_count := 0; end;
  begin select count(*) into v_bike_brand_count from bike_brands where tenant_id = p_tenant_id; exception when undefined_table then v_bike_brand_count := 0; end;
  begin select count(*) into v_bike_model_count from bike_models where tenant_id = p_tenant_id; exception when undefined_table then v_bike_model_count := 0; end;
  begin select count(*) into v_online_order_count from online_orders where tenant_id = p_tenant_id; exception when undefined_table then v_online_order_count := 0; end;
  begin select count(*) into v_website_banner_count from website_banners where tenant_id = p_tenant_id; exception when undefined_table then v_website_banner_count := 0; end;
...
create unique index if not exists idx_compat_component_aliases_unique on compat_component_aliases(tenant_id, component_type_id, lower(alias));
...
create unique index if not exists idx_compat_attribute_options_unique on compat_attribute_options(tenant_id, attribute_id, lower(value_key));
