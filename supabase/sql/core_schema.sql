...
-- Drop old function first (if exists with different return type)
...
-- Website pages (GrapesJS WYSIWYG editor - stores HTML/CSS for each page)
...
-- Website pages: RLS enabled (tenant-filtered policies added later)
...
-- Website Pages: Tenant isolation
do $$ begin
  create policy "website_pages_select" on website_pages for select using (tenant_id = public.user_tenant_id());
  create policy "website_pages_insert" on website_pages for insert with check (tenant_id = public.user_tenant_id());
  create policy "website_pages_update" on website_pages for update using (tenant_id = public.user_tenant_id());
  create policy "website_pages_delete" on website_pages for delete using (tenant_id = public.user_tenant_id());
