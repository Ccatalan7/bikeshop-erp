...
create table if not exists stock_adjustments (
  ...
  adjustment_type text not null check (adjustment_type in ('manual', 'correction', 'initial', 'damage', 'loss', 'found', 'import', 'purchase', 'count_gain', 'count_loss', 'theft', 'internal_use')),
...
alter table stock_adjustments alter column adjustment_date set default now();
...
create table if not exists suppliers (
  ...
  ocr_template jsonb not null default '{}'::jsonb,
...
-- Table: bike_profiles
-- Structured intake + technical baseline for each bike
...
-- Bike Profiles: Tenant isolation
do $$ begin
  create policy "bike_profiles_select" on bike_profiles for select to authenticated using (tenant_id = public.user_tenant_id());
  create policy "bike_profiles_insert" on bike_profiles for insert to authenticated with check (tenant_id = public.user_tenant_id());
  create policy "bike_profiles_update" on bike_profiles for update to authenticated using (tenant_id = public.user_tenant_id());
  create policy "bike_profiles_delete" on bike_profiles for delete to authenticated using (tenant_id = public.user_tenant_id());
  raise notice '✓ Created RLS policies for bike_profiles';
exception
  when undefined_table then raise notice '⚠ Table bike_profiles does not exist yet';
  when undefined_column then raise notice '⚠ Column tenant_id missing in bike_profiles';
