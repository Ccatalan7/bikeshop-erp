...
-- 5) credit 2101 for the invoice total.
--
-- IMPORTANT:
...
--
-- TAX SCOPE:
-- This verifier intentionally uses 'no_tax' so it isolates the workshop
-- consumable vs inventory posting split without mixing in IVA calculations.
-- Purchase IVA is still supported by the accounting function: when tax > 0,
-- account 2120 (IVA Credito Fiscal) is debited for p_invoice.tax.
-- In this script, the 2120 line is expected to be zero.
--
-- NOTE:
...
drop table if exists tmp_wc_purchase_context;
drop table if exists tmp_wc_purchase_test;

create temporary table tmp_wc_purchase_context as
with params as (
  select
...
select
  ...
  ((x.inventory_qty * x.inventory_unit_cost) + (x.workshop_qty * x.workshop_unit_cost))::numeric(12,2) as invoice_total,
...
create temporary table tmp_wc_purchase_test as
with inserted_invoice as (
  ...
  from tmp_wc_purchase_context c
  ...
  jsonb_build_object(
    ...
    coalesce(
      (
        select to_jsonb(p)
        from public.products p
        join tmp_wc_purchase_context c
    ...
    jsonb_build_object(
      ...
      coalesce(
        (
          ...
          from public.journal_lines jl
          join tmp_wc_purchase_context c on true
          where jl.entry_id in (
            select je.id
            from public.journal_entries je
            join tmp_wc_purchase_test t
