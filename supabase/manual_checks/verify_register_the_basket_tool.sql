select
  ...
  ) = 4 then 1 else 0 end) as canasta_anunciada,
  1 / (case when (
    select max_result_count
    from assistant_runtime.assistant_tool_receipt_contract_internal_v1(
      'rank_purchase_suppliers')
  ) = 5 then 1 else 0 end) as la_de_una_frase_sigue,
  1 / (case when (
    ...
    where not exists (
      select 1 from assistant_runtime.assistant_tool_receipt_contract_internal_v1(
        previa.tool_name)
    )
