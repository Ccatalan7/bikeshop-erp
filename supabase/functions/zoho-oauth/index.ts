...
function handleOAuthRedirect(url: URL) {
  ...
  if (error) {
    if (isMobile) {
      return Response.redirect(`${mobileDeepLink}?provider=zoho&error=${encodeURIComponent(error)}`, 302)
    }
  ...
  if (code) {
    if (isMobile) {
      return Response.redirect(`${mobileDeepLink}?provider=zoho&oauth_code=${encodeURIComponent(code)}`, 302)
    }
