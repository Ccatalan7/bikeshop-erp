...
function safeStorageSource(rawUrl: string, supabaseUrl: string) {
  try {
    ...
    if (source.protocol !== "https:" || source.host !== project.host) {
      return false;
    }
    const allowedRoots = [
      "/storage/v1/object/public/vinabike-assets/website/media/",
      "/storage/v1/object/public/vinabike-assets/website-images/",
      "/storage/v1/object/public/vinabike-assets/website/blocks/",
      "/storage/v1/render/image/public/vinabike-assets/website/media/",
      "/storage/v1/render/image/public/vinabike-assets/website-images/",
      "/storage/v1/render/image/public/vinabike-assets/website/blocks/",
    ];
    return allowedRoots.some((root) => source.pathname.startsWith(root));
  } catch (_) {
    return false;
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    const objectPath = "website/media/sources/" +
      tenantId +
      "/smart-no-bg-" +
      crypto.randomUUID() +
      ".png";
    const { error: uploadError } = await adminClient.storage
      ...
      .upload(objectPath, resultBytes, {
        ...
        metadata: {
          ...
          tenant_id: tenantId,
          source_bytes: resultBytes.length,
        },
      });
    if (uploadError) {
