...
export function createDefaultAgentToolRegistry(options: { publicResearch?: boolean } = {}) {
  return new AgentToolRegistry([
    ...
    readTool(
      ...
      "Resuelve una LISTA completa en una sola llamada y decide si conviene un proveedor o repartir en dos. Úsala en cuanto el operador nombre dos o más cosas —«necesito rayos 27.5, cámaras 29 y cadenas de 11v»—, NUNCA llames rank_purchase_suppliers una vez por línea: eso agota el presupuesto del turno y la respuesta se pierde (medido). Devuelve los proveedores ordenados por cuántas líneas EXACTAS cubre cada uno según el historial real de compras, con `coveredNeeds` de `totalNeeds` y `coveredList`. Una búsqueda ampliada viaja aparte en `approximateNeeds` y `approximateList`: es una sugerencia parecida y JAMÁS completa la línea exacta. La fila de rango 1 trae además `missingList` —lo que ese proveedor NO cubre exactamente— y `complementSupplierName`, que es el segundo proveedor que completa la lista: ésa es la decisión de repartir, ya calculada; dila tal cual y no la recalcules. Si `missingList` viene nulo, un solo proveedor cubre todo y se dice así. Cubrir significa que se lo hemos comprado exacto, nunca que tenga stock hoy.",
